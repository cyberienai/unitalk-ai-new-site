...
export function AcademyHero({ kicker, title, body }: { kicker:string; title:string; body?:string }) { return <section className="academy-paper-grid px-5 pb-16 pt-36"><div className="academy-shell"><AcademyKicker>{kicker}</AcademyKicker><h1 className="academy-display mt-7 max-w-6xl">{title}</h1>{body&&<p className="mt-7 max-w-3xl text-lg leading-8 text-[#625b50]">{body}</p>}</div></section> }
