...
export function AcademyOnboarding({firstName,initialMission,initialPath}:{firstName:string;initialMission?:string;initialPath?:string}){
  ...
  return <main className="min-h-screen bg-[#F3EFE6] text-[#1C1A17] lg:grid lg:grid-cols-[.78fr_1.22fr]">
    <aside className="relative overflow-hidden bg-[#181615] px-6 py-10 text-white sm:px-10 lg:min-h-screen lg:p-12"><div aria-hidden className="absolute inset-0 opacity-[.06] [background-image:linear-gradient(#fff_1px,transparent_1px),linear-gradient(90deg,#fff_1px,transparent_1px)] [background-size:72px_72px]"/><div className="relative flex h-full flex-col"><p className="font-mono text-[10px] font-black uppercase tracking-[.2em] text-[#F2A4C5]">Alma · Onboarding Academy</p><h1 className="mt-6 font-sf text-[clamp(2.8rem,5vw,5.5rem)] font-semibold leading-[.88] tracking-[-.07em]">Bonjour {firstName}.<br/><span className="text-[#F2A4C5]">Que voulez-vous savoir livrer ?</span></h1><p className="mt-6 max-w-xl text-[16px] leading-8 text-[#CFC6B8]">Je personnalise votre point de départ. Vous retrouverez ensuite vos formations dans le même compte Unitalk.</p><div className="relative mt-10 aspect-[4/3] max-w-md overflow-hidden border border-white/15 lg:mt-auto"><Image src="/alma-avatar.png" alt="Alma, guide Unitalk Academy" fill priority className="object-cover object-top"/><div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-[#181615] via-[#181615]/80 to-transparent p-5 pt-16"><strong className="text-sm">Même Alma. Contexte pédagogique dédié.</strong><p className="mt-1 text-xs text-[#CFC6B8]">Aucun document opérationnel n’est transféré sans votre accord.</p></div></div></div></aside>
