...
export function FloatingAlmaWidget() {
  ...
  return <>
    {!hidden&&<div className="fixed bottom-6 right-6 z-40"><motion.button type="button" onClick={toggleAlma} initial={{scale:0,opacity:0}} animate={{scale:1,opacity:1}} transition={{delay:.4,duration:.25}} aria-label={isOpen?t.close:t.tooltip} title={isOpen?t.close:t.tooltip} className={`overflow-hidden rounded-full bg-[#F3EFE6] shadow-[0_12px_32px_-8px_rgba(28,26,23,.35)] ring-2 ring-[#D10E63]/40 ${missionDetail?'size-12':'size-16'}`}>{isOpen?<span className="flex size-full items-center justify-center bg-[#D10E63] text-white"><X className="size-5"/></span>:<img src="/alma-avatar.png" alt="Alma" className="size-full object-cover"/>}</motion.button></div>}
    <AnimatePresence>{isOpen&&<motion.div initial={{opacity:0,scale:.92,y:16}} animate={{opacity:1,scale:1,y:0}} exit={{opacity:0,scale:.94,y:12}} className={`fixed right-4 z-50 w-[min(390px,calc(100vw-2rem))] overflow-hidden rounded-[26px] border border-[#D8D0C2] bg-[#FAF8F3] shadow-[0_30px_90px_-30px_rgba(28,26,23,.55)] ${hidden?'bottom-4':'bottom-28'}`}>
      <header className="flex items-center justify-between border-b border-[#E4DCCC] px-5 py-4"><div className="flex items-center gap-3"><img src="/alma-avatar.png" alt="" className="size-10 rounded-full object-cover ring-2 ring-[#D10E63]/25"/><div><p className="text-sm font-semibold">Alma</p><p className="text-[11px] text-[#857C6E]">{t.role}</p></div></div><button type="button" onClick={closeAlma} aria-label={t.close} className="flex size-9 items-center justify-center rounded-full hover:bg-[#EAE3D4]"><X className="size-4"/></button></header>
      <div className="p-5"><h2 className="text-2xl font-semibold tracking-[-.035em]">{t.title}</h2><textarea value={need} onChange={event=>setNeed(event.target.value)} rows={4} placeholder={t.placeholder} className="mt-4 w-full resize-none rounded-2xl border border-[#D8D0C2] bg-white p-4 text-sm leading-6 outline-none placeholder:text-[#A79E8E] focus:border-[#D10E63] focus:ring-2 focus:ring-[#D10E63]/15"/><button type="button" onClick={continueFlow} disabled={!need.trim()} className="mt-4 inline-flex min-h-12 w-full items-center justify-center gap-2 rounded-full bg-[#D10E63] px-6 text-sm font-bold text-white disabled:opacity-40">{t.continue}<ArrowRight className="size-4"/></button><Link href={`/connexion?redirect=${encodeURIComponent(pathname)}`} onClick={closeAlma} className="mt-4 block text-center text-sm font-bold text-[#4E483F] underline decoration-[#D10E63]/30 underline-offset-4">{t.signIn}</Link><p className="mt-4 text-center text-[10px] leading-5 text-[#857C6E]">{t.note}</p></div>
