      ...
      <section className="hero-viewport relative flex items-center overflow-hidden border-b border-[#CFC5B5] pb-14 pt-24 sm:pb-16 sm:pt-28">
        ...
        <div className="editorial-shell relative grid items-center gap-12 lg:grid-cols-[1fr_1fr] lg:gap-16">
          <div className="max-w-2xl">
            ...
            <p className="mx-auto mt-6 max-w-xl text-balance text-center text-base leading-8 text-[#4E483F] sm:mx-0 sm:text-left md:text-lg">{t.lead}</p>
            <div className="mt-8 flex flex-col items-center gap-3 sm:flex-row"><Link href="/decouvrir?source=hermes" className="inline-flex min-h-12 items-center justify-center rounded-full bg-[#D10E63] px-7 text-sm font-bold text-white">{t.primary}<ArrowRight className="ml-2 size-4"/></Link><a href="#responsabilites" className="inline-flex min-h-12 items-center justify-center rounded-full border border-[#191715] px-7 text-sm font-bold">{t.secondary}</a></div>
...
const COPY = {
  fr: {
    title:'Hermes fonctionne.', accent:'Votre équipe travaille.', lead:'Unitalk déploie, sécurise et maintient l’environnement de vos Collaborateurs IA pour que votre entreprise se concentre sur leurs missions.', primary:'Évaluer mon besoin d’hébergement', secondary:'Qui contrôle quoi ?', heroNote:'Configuration, isolation et engagements de service selon l’offre et l’hébergement choisis.',
    consoleLabel:'État opérationnel d’une instance Hermes', active:'Active', consoleRows:[['Instance','Emma'],['Version Hermes','Stable · suivie'],['Région','France'],['Dernière sauvegarde','Aujourd’hui · 03:14',true],['État du service','Opérationnel',true]], monitoring:'Supervision', enabled:'Active', updates:'Mises à jour', controlled:'Contrôlées',
    ...
    choiceKicker:'Hébergement', choiceTitle:'Unitalk AI Cloud ou un fournisseur compatible.', choiceBody:'Choisissez selon vos exigences de localisation, de confidentialité, de disponibilité et de budget. La compatibilité et le niveau d’exploitation sont confirmés avant déploiement.', cloud:'Unitalk déploie et opère l’environnement dans son cloud administré, selon la région et le niveau de service disponibles dans votre offre.', providerTitle:'Votre hébergeur', provider:'Un fournisseur compatible peut être étudié. Les responsabilités, accès techniques, coûts et engagements sont alors précisés avant déploiement.', serversCta:'Comparer les options d’hébergement',
  ...
  en: {
    title:'Hermes runs.', accent:'Your team gets work done.', lead:'Unitalk deploys, secures and maintains your AI Collaborators’ environment so your organization can focus on their missions.', primary:'Assess my hosting needs', secondary:'Who controls what?', heroNote:'Configuration, isolation and service commitments depend on the selected plan and hosting provider.',
    consoleLabel:'Hermes instance operational status', active:'Active', consoleRows:[['Instance','Emma'],['Hermes version','Stable · managed'],['Region','France'],['Latest backup','Today · 03:14',true],['Service status','Operational',true]], monitoring:'Monitoring', enabled:'Active', updates:'Updates', controlled:'Controlled',
    ...
    choiceKicker:'Hosting', choiceTitle:'Unitalk AI Cloud or a compatible provider.', choiceBody:'Choose according to location, confidentiality, availability and budget requirements. Compatibility and operating scope are confirmed before deployment.', cloud:'Unitalk deploys and operates the environment in its managed cloud, according to the region and service level available in your plan.', providerTitle:'Your hosting provider', provider:'A compatible provider can be assessed. Responsibilities, technical access, costs and commitments are then defined before deployment.', serversCta:'Compare hosting options',
