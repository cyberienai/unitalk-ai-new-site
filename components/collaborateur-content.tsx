...
const COPY = {
  fr: {
    ...
    evolutionBody: 'Hugo commence avec un profil commercial. Ajoutez ensuite de nouvelles compétences, applications et profils métier selon le travail confié.',
...
const PERSONAS = {
  ...
  hugo: {
    ...
    composer: { fr: 'Hugo trouve et qualifie vos prospects.', en: 'Hugo finds and qualifies your prospects.' },
    placeholder: { fr: 'Précisez simplement les prospects recherchés…', en: 'Simply describe the prospects you are looking for…' },
    ...
    proofTitle: { fr: <>Hugo prospecte.<br/><span className="whitespace-nowrap">Votre équipe décide.</span></>, en: <>Hugo prospects.<br/><span className="whitespace-nowrap">Your team decides.</span></> },
...
export function CollaborateurContent({ page }: { page: CollaboratorPage; equipmentId?: string }) {
  ...
  function personalizeHugo() {
    const text = lang === 'fr' ? 'Trouver et qualifier mes prospects' : 'Find and qualify my prospects'
    const draftId = `draft_${crypto.randomUUID()}`
    try { localStorage.setItem(`unitalk_mission_${draftId}`, JSON.stringify({ text, collaborator: detail.slug, createdAt: Date.now() })) } catch {}
    router.push(`/decouvrir?draft=${encodeURIComponent(draftId)}&collaborateur=${encodeURIComponent(detail.slug)}&source=profile-store`)
  }

  return <main className="collaborator-profile-page overflow-hidden bg-[#F3EFE6] text-[#1C1A17]">
      ...
      <div className="editorial-shell relative w-full">
        <div className="grid gap-10 lg:grid-cols-[1.02fr_.98fr] lg:items-center lg:gap-14">
          <div>
            ...
            {detail.slug !== 'hugo' && <Link href="#alma-profile" className="mt-8 inline-flex min-h-12 items-center justify-center rounded-full bg-[#181615] px-6 text-sm font-bold text-white shadow-[0_14px_34px_-20px_rgba(28,26,23,.7)] hover:bg-[#2A2622]">{lang==='fr'?`Adapter le profil ${detail.role.fr} avec Alma`:`Tailor the ${detail.role.en} profile with Alma`}</Link>}
          </div>
          <div id="alma-profile" className="scroll-mt-24">{detail.slug === 'hugo' ? <article className="relative overflow-hidden rounded-[26px] border border-white/10 bg-[#17130F] p-6 text-[#F8F1E7] shadow-[0_34px_80px_-28px_rgba(23,19,15,.65)] sm:p-7"><div aria-hidden className="absolute inset-x-10 top-0 h-px bg-gradient-to-r from-transparent via-[#F15B9B] to-transparent"/><div className="flex items-center gap-3"><Image src="/alma-avatar.png" alt="Alma" width={44} height={44} className="size-11 rounded-full object-cover ring-2 ring-[#D10E63]/35"/><div><p className="font-sf font-semibold">Alma</p><p className="text-xs text-[#D6CABD]">{t.composerRole}</p></div></div><div className="py-10 sm:py-12"><h2 className="text-balance font-sf text-[clamp(1.75rem,3vw,2.5rem)] font-bold leading-[1.02] tracking-[-.04em]">{persona.composer[lang]}</h2><p className="mt-4 max-w-md text-[15px] font-medium leading-7 text-[#D6CABD]">{lang==='fr'?'Profil commercial recommandé · Mission prête à personnaliser':'Recommended sales profile · Mission ready to customize'}</p></div><button type="button" onClick={personalizeHugo} className="inline-flex min-h-12 w-full items-center justify-center gap-2 rounded-full bg-[#E51872] px-6 text-sm font-bold text-white transition-colors hover:bg-[#F02A82]">{t.personalizeHugo}<ArrowRight className="size-4"/></button></article> : <AlmaMissionComposer value={missionRequest} onChange={setMissionRequest} onSubmit={submitMission} title={persona.composer[lang]} body="" role={t.composerRole} placeholder={persona.placeholder[lang]} submitLabel={lang==='fr'?`Confier cette mission à ${detail.name}`:`Assign this mission to ${detail.name}`} starters={persona.examples[lang]} listening={false} onToggleListening={() => {}} voiceSupported={false} voiceStartLabel="" voiceStopLabel="" compactMobile compactDesktop />}<ul className="mt-4 flex flex-wrap gap-x-5 gap-y-2">{t.trialProofs.map(proof => <li key={proof} className="flex items-center gap-2 text-xs font-semibold text-[#625B50]"><Check className="size-3.5 text-[#D10E63]"/>{proof}</li>)}</ul></div>
        </div>
    ...
    <section className="collaborator-identity-section py-20 sm:py-24">
      <div className="editorial-shell grid gap-10 lg:grid-cols-[1.12fr_.88fr] lg:items-center lg:gap-12"><div><Kicker>{t.identityKicker}</Kicker><h2 className="mt-5 max-w-[760px] text-[clamp(2.35rem,4vw,4rem)] font-bold leading-[.98] tracking-[-.05em]">{lang==='fr'?<><span className="block whitespace-nowrap">{detail.name} travaille avec</span><span className="block whitespace-nowrap">sa propre identité</span><span className="block whitespace-nowrap">et ses propres accès.</span></>:<><span className="block whitespace-nowrap">{detail.name} works with</span><span className="block whitespace-nowrap">their own identity</span><span className="block whitespace-nowrap">and access.</span></>}</h2><p className="mt-6 max-w-xl text-[16px] leading-8 text-[#4E483F]">{lang==='fr'?`${detail.name} appartient à votre entreprise, indépendamment de la personne chargée de sa supervision. Si cette personne quitte l’organisation, son identité, sa mémoire et son savoir-faire restent dans l’entreprise.`:`${detail.name} belongs to your organization, independently of the person supervising them. If that person leaves, identity, memory and know-how remain within the organization.`}</p><Link href="/collaborateurs-ia" className="mt-7 inline-flex items-center gap-2 text-sm font-bold text-[#B00C54]">{t.identityCta}<ArrowRight className="size-4"/></Link></div><div className="w-full lg:max-w-[540px] lg:justify-self-end"><IdentityCard detail={detail} lang={lang} labels={t.identityCard}/></div></div>
    </section>
    ...
    <section className="collaborator-evolution-section py-20 sm:py-24"><div className="editorial-shell grid gap-10 lg:grid-cols-[1.1fr_.9fr] lg:items-center lg:gap-12"><div><Kicker>{t.evolutionKicker}</Kicker><h2 className="mt-5 text-[clamp(2.35rem,4vw,4rem)] font-bold leading-[.98] tracking-[-.05em]">{lang==='fr'?<><span className="block whitespace-nowrap">{detail.name} accomplit</span><span className="block whitespace-nowrap">une mission.</span><span className="block whitespace-nowrap text-[#D10E63]">Son expérience</span><span className="block whitespace-nowrap text-[#D10E63]">reste.</span></>:<><span className="block whitespace-nowrap">{detail.name} completes</span><span className="block whitespace-nowrap">a mission.</span><span className="block whitespace-nowrap text-[#D10E63]">The experience</span><span className="block whitespace-nowrap text-[#D10E63]">remains.</span></>}</h2><p className="mt-6 max-w-xl text-[16px] leading-8 text-[#4E483F]">{lang==='fr'?`${detail.name} commence avec un profil ${detail.role.fr.toLowerCase()}. Ajoutez ensuite de nouvelles compétences, applications et profils métier selon le travail confié.`:`${detail.name} starts with a ${detail.role.en.toLowerCase()} profile. Add new skills, applications and job profiles as new work is assigned.`}</p><Link href="/marketplace" className="mt-7 inline-flex items-center gap-2 text-sm font-bold text-[#B00C54]">{lang==='fr'?'Explorer la Marketplace':'Explore the Marketplace'}<ArrowRight className="size-4"/></Link></div><article className="w-full overflow-hidden rounded-[28px] bg-[#181615] p-7 text-white sm:p-9 lg:max-w-[560px] lg:justify-self-end"><div className="flex items-center gap-4 border-b border-white/10 pb-6"><Image src={detail.avatar} alt="" width={58} height={58} className="size-[58px] rounded-full object-cover ring-2 ring-[#F2A4C5]/25"/><div><p className="text-xl font-semibold">{detail.name}</p><p className="mt-1 text-xs text-[#AFA397]">{lang==='fr'?'Même identité, nouvelles capacités':'Same identity, new capabilities'}</p></div></div><ul className="mt-6 grid gap-3 sm:grid-cols-2">{t.evolutionItems.map(item => <li key={item} className="flex gap-3 rounded-2xl border border-white/10 bg-white/[.04] p-4 text-sm font-semibold"><Check className="mt-0.5 size-4 shrink-0 text-[#F2A4C5]"/>{item}</li>)}</ul></article></div></section>

    {missions.length>0&&<section id="missions" className="scroll-mt-24 bg-[#FAF8F3] py-20 sm:py-24"><div className="editorial-shell"><h2 className="text-[clamp(2.2rem,4.2vw,4rem)] font-bold leading-[.98] tracking-[-.05em] lg:whitespace-nowrap">{lang==='fr'?`Missions prêtes à l’emploi avec ${detail.name}`:`Ready-to-use missions with ${detail.name}`}</h2><div className="mt-10 grid gap-4 md:grid-cols-2">{missions.slice(0,4).map(mission => <Link key={mission.slug} href={`/missions/${mission.slug}`} className="group rounded-2xl border border-[#D8D0C2] bg-white p-6 transition hover:-translate-y-1 hover:border-[#D10E63]/40"><span className={`inline-flex rounded-full px-2.5 py-1 text-[10px] font-bold uppercase tracking-[.08em] ${mission.status==='available'?'bg-[#267A48]/10 text-[#267A48]':mission.status==='on-setup'?'bg-[#D10E63]/10 text-[#B00C54]':'bg-[#EAE3D4] text-[#625B50]'}`}>{STATUS_LABELS[mission.status][lang]}</span><h3 className="mt-4 text-xl font-semibold">{mission.title[lang]}</h3><p className="mt-3 text-sm leading-6 text-[#625B50]">{mission.objective[lang]}</p><span className="mt-6 inline-flex items-center gap-2 text-sm font-bold text-[#B00C54]">{lang==='fr'?'Voir la mission':'View mission'}<ArrowRight className="size-4 transition-transform group-hover:translate-x-1"/></span></Link>)}</div><Link href={`/missions?collaborateur=${encodeURIComponent(detail.slug)}&vue=toutes`} className="mt-8 inline-flex items-center gap-2 text-sm font-bold text-[#B00C54]">{lang==='fr'?`Voir toutes les missions de ${detail.name}`:`View all ${detail.name} missions`}<ArrowRight className="size-4"/></Link></div></section>}
