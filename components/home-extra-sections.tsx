...
export function AIGatewaySection() {
  return (
    <section className="w-full bg-[#EFE9DC] px-5 py-20 sm:px-6 sm:py-28 lg:px-8">
      <div className="mx-auto max-w-4xl text-center">
        <motion.h2
          className="mb-6 text-3xl font-bold leading-tight text-[#1C1A17] sm:text-4xl lg:text-[2.75rem]"
        ...
        <p className="mx-auto mb-10 max-w-2xl text-lg leading-relaxed text-[#4E483F]">
          Avec <span className="font-semibold text-[#1C1A17]">Unitalk AI Gateway</span>, vos humains
          et vos agents accèdent aux meilleurs modèles depuis un même endroit.
        </p>
        ...
        <div className="mb-10 flex flex-wrap justify-center gap-2.5">
          {GATEWAY_MODELS.map((m, i) => (
            <motion.span
              key={m}
              className="inline-flex items-center rounded-full border border-[#DcD4C4] bg-[#F3EFE6] px-4 py-2 text-sm font-medium text-[#1C1A17]"
        ...
        <div className="mx-auto mb-8 max-w-md space-y-1.5 text-sm text-[#4E483F]">
          <p>Pas de clés API à gérer.</p>
          <p>Pas de comptes IA empilés.</p>
          <p>Pas de facturation modèle par utilisateur.</p>
        </div>

        <p className="mx-auto max-w-2xl text-xl font-semibold leading-snug text-[#1C1A17]">
          {"L'IA devient une ressource d'entreprise."}
          <br />
          {"Pas une pile d'abonnements individuels."}
        </p>
      </div>
    </section>
  )
...
const SOVEREIGNTY = [
  ...
  { icon: Repeat, title: 'Votre capacité à changer de modèle' },
]
      ...
      <div className="mx-auto max-w-5xl">
        <div className="mx-auto mb-12 max-w-2xl text-center">
          ...
          <p className="text-lg leading-relaxed text-[#4E483F]">
            Avec Unitalk, vous gardez la maîtrise de ce qui compte :
          </p>
        </div>
        ...
        <div className="mb-12 grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
          {SOVEREIGNTY.map((s, i) => {
            ...
            return (
              <motion.div
                key={s.title}
                className="rounded-2xl border border-[#DcD4C4] bg-[#EFE9DC] p-6"
                initial={{ opacity: 0, y: 20 }}
                whileInView={{ opacity: 1, y: 0 }}
                viewport={{ once: true }}
                transition={{ duration: 0.4, delay: i * 0.08 }}
              >
                <span className="mb-4 flex h-10 w-10 items-center justify-center rounded-lg bg-[#4F5BD5]/12 text-[#4F5BD5]">
        ...
        <div className="mx-auto max-w-2xl text-center">
          <p className="mb-6 text-base leading-relaxed text-[#4E483F]">
...
const OFFERS = [
  ...
  {
    name: 'Teams',
    subtitle: 'Un Collaborateur IA pour chaque membre de l\u2019équipe.',
  ...
  {
    name: 'Business',
    subtitle: 'Vos agents IA, vos apps métiers, votre serveur privé.',
    text: 'Déployez des Collaborateurs IA dans une infrastructure privée avec vos données, vos API et vos applications open source.',
    points: [
      'Serveur IA privé',
  ...
  return (
    <section id="offres" className="w-full bg-[#EFE9DC] px-5 py-20 sm:px-6 sm:py-28 lg:px-8">
      <div className="mx-auto max-w-6xl">
        <motion.h2
          className="mb-12 text-center text-3xl font-bold leading-tight text-[#1C1A17] sm:text-4xl lg:text-[2.75rem]"
          initial={{ opacity: 0, y: 20 }}
          whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true }}
          transition={{ duration: 0.5 }}
          ...
          {OFFERS.map((o, i) => (
            <motion.div
              ...
              className={`flex flex-col rounded-2xl border p-8 ${
                o.featured
                  ? 'border-[#D10E63]/40 bg-[#F3EFE6] shadow-lg'
                  : 'border-[#DcD4C4] bg-[#F3EFE6]'
              }`}
              initial={{ opacity: 0, y: 20 }}
              whileInView={{ opacity: 1, y: 0 }}
              viewport={{ once: true }}
              ...
              {o.badge && (
                <span
                  className={`mb-4 inline-flex w-fit items-center rounded-full px-3 py-1 text-xs font-medium ${
                    o.featured ? 'bg-[#D10E63]/12 text-[#D10E63]' : 'bg-[#4F5BD5]/12 text-[#4F5BD5]'
                  }`}
                >
                  {o.badge}
                </span>
  ...
  return (
    <section className="w-full bg-[#F3EFE6] px-5 py-20 sm:px-6 sm:py-28 lg:px-8">
      <div className="mx-auto max-w-3xl">
        <h2 className="mb-12 text-center text-3xl font-bold text-[#1C1A17] sm:text-4xl">
          Questions fréquentes
        </h2>
        <div className="space-y-3">
          {FAQ.map((item, i) => (
            <div key={i} className="rounded-2xl border border-[#DcD4C4] bg-[#EFE9DC] overflow-hidden">
              <button
...
export function FinalCtaSection() {
  return (
    <section className="w-full bg-[#1C1A17] px-5 py-24 sm:px-6 sm:py-32 lg:px-8">
      <div className="mx-auto max-w-3xl text-center">
        <motion.h2
          className="mb-6 text-3xl font-bold leading-tight text-[#FBF9F3] sm:text-4xl lg:text-5xl"
          initial={{ opacity: 0, y: 20 }}
          whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true }}
          transition={{ duration: 0.5 }}
        ...
        <p className="mx-auto mb-6 max-w-2xl text-lg leading-relaxed text-[#C9C1B4]">
          Un vrai collaborateur IA avec une identité, une intelligence, une mémoire, des compétences,
          des outils, des ressources et des instructions de travail.
        </p>

        <div className="mb-10 space-y-1 text-sm text-[#A79F91]">
          <p>Guidé par Alma en 7 étapes.</p>
          <p>Mise en service en moins de 15 minutes.</p>
