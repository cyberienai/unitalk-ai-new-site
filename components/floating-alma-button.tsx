...
export function FloatingAlmaButton() {
  // Opens by default once the hero has settled; user can close and reopen.
  ...
  return (
    <div className="fixed bottom-4 sm:bottom-6 md:bottom-8 right-4 sm:right-6 md:right-8 z-40 flex flex-col items-end gap-3">
      {/* Launcher panel */}
      <AnimatePresence>
        {isOpen && (
          <motion.div
            className="w-[calc(100vw-2rem)] max-w-sm overflow-hidden rounded-3xl border border-[rgba(255,255,255,0.08)] bg-[#0A0A0A] shadow-2xl"
          ...
          >
            ...
            <div className="flex items-center gap-3 border-b border-[rgba(255,255,255,0.06)] bg-[#111111] px-4 py-3.5">
              ...
              <button
                ...
                className="flex h-8 w-8 items-center justify-center rounded-full text-[#8E8E93] transition-colors hover:bg-white/5 hover:text-white"
                aria-label="Fermer"
              >
      ...
      {/* Floating button (reopens the panel) */}
      {!isOpen && (
        <motion.button
          className="relative h-14 w-14 flex-shrink-0 rounded-full border border-[rgba(255,0,153,0.4)] bg-[#161616] transition-colors hover:bg-[#1E1E1E] flex items-center justify-center"
          style={{ boxShadow: '0 4px 24px rgba(255, 0, 153, 0.2)' }}
