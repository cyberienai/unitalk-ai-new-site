...
const SELECTED_SLUGS = [
  'consultant-strategie-digitale', 'chef-projet-digital', 'responsable-seo', 'gestionnaire-campagnes-publicitaires',
  'responsable-acquisition', 'responsable-editorial', 'redacteur-web', 'community-manager',
  'responsable-crm', 'webmaster', 'analyste-web', 'integrateur-no-code-automatisation',
] as const

const MAIN_CATEGORIES = ['conseil-projet', 'acquisition', 'contenu-social', 'crm-cycle-vie', 'web-ecommerce', 'data-mesure', 'design-creation', 'developpement-integration', 'qualite-exploitation'] as const
...
export function ProfilsFinalContent() {
  ...
  function handleCategoryClick(event: React.MouseEvent<HTMLAnchorElement>, next: string) {
  ...
  return (
    <main className="bg-[#F3EFE6] text-[#1C1A17]">
      <section className="px-5 pb-6 pt-20 sm:px-8 sm:pt-24 lg:pb-4 lg:pt-20">
        <div className="mx-auto max-w-[1240px]">
          <div className="grid gap-8 lg:grid-cols-[1.08fr_.92fr] lg:items-center">
            <div>
              ...
              <h1 className="mt-3 font-sf text-[42px] font-bold leading-[.97] tracking-[-.05em] sm:text-[54px]"><span className="block">{fr ? 'Ajoutez une responsabilité.' : 'Add a responsibility.'}</span><span className="block text-[#6E665A]">{fr ? 'Gardez la même identité.' : 'Keep the same identity.'}</span></h1>
              <p className="mt-4 max-w-2xl text-[16px] leading-7 text-[#4E483F]">{fr ? 'Chaque expert de l’agence peut travailler avec un Collaborateur IA qui partage ses méthodes, prépare ses missions et progresse avec l’entreprise.' : 'Every agency expert can work with an AI Collaborator that shares their methods, prepares missions and grows with the company.'}</p>
          ...
          <div id="catalogue" className="mt-5 scroll-mt-24 border-t border-[#1C1A17]/15 pt-5">
            <div className="flex gap-2 overflow-x-auto pb-2 [scrollbar-width:none]"> <Category href="/collaborateurs-ia/profils-metier" active={category === 'selection'} onClick={event => handleCategoryClick(event, 'selection')}>Sélection</Category>{MAIN_CATEGORIES.map(key => <Category key={key} href={`/collaborateurs-ia/profils-metier?categorie=${key}`} active={category === key} onClick={event => handleCategoryClick(event, key)}>{shortLabel(key, lang)}</Category>)}<button onClick={() => setAllOpen(value => !value)} aria-expanded={allOpen} className="inline-flex h-9 shrink-0 items-center justify-center gap-1.5 rounded-full border border-[#D10E63]/50 bg-white px-3.5 text-[12px] font-semibold leading-none text-[#B00C54] outline-none focus-visible:ring-2 focus-visible:ring-[#D10E63] focus-visible:ring-offset-2">{fr ? 'Toutes les catégories' : 'All categories'}<ChevronDown aria-hidden className={`h-3.5 w-3.5 ${allOpen ? 'rotate-180' : ''}`}/></button></div>
...
function Category({href,active,onClick,children}:{href:string;active:boolean;onClick:(event:React.MouseEvent<HTMLAnchorElement>)=>void;children:React.ReactNode}){return <a href={href} onClick={onClick} aria-current={active?'page':undefined} className={`inline-flex h-9 shrink-0 items-center justify-center rounded-full border px-3.5 text-[12px] font-semibold leading-none outline-none focus-visible:ring-2 focus-visible:ring-[#D10E63] focus-visible:ring-offset-2 ${active?'border-[#D10E63] bg-[#D10E63] text-white':'border-[#DED6C8] bg-white text-[#4E483F] hover:text-[#D10E63]'}`}>{children}</a>}
function shortLabel(key:string,lang:'fr'|'en'){const labels:Record<string,{fr:string;en:string}>={'conseil-projet':{fr:'Conseil et projet',en:'Consulting & project'},acquisition:{fr:'Acquisition',en:'Acquisition'},'contenu-social':{fr:'Contenu',en:'Content'},'crm-cycle-vie':{fr:'CRM',en:'CRM'},'web-ecommerce':{fr:'Web',en:'Web'},'data-mesure':{fr:'Données',en:'Data'},'design-creation':{fr:'Design',en:'Design'},'developpement-integration':{fr:'Développement',en:'Development'},'qualite-exploitation':{fr:'Qualité',en:'Quality'}};return labels[key]?.[lang]??DOMAIN_LABELS[key]?.[lang]??key}
...
function genericLabel(profile:StoreItem,lang:'fr'|'en'){return profile.labels?.neutral?.[lang] ?? profile.labels?.default?.[lang] ?? profile.name[lang]}
...
function Continuity(){return <section className="px-5 py-12 sm:px-8"><div className="mx-auto max-w-[1100px]"><p className="label">Une seule identité IA. Plusieurs responsabilités.</p><h2 className="mt-3 font-sf text-[38px] font-bold">Le même Lucas. Une nouvelle responsabilité.</h2><p className="mt-4 max-w-3xl text-[16px] leading-7 text-[#4E483F]">Ajoutez un profil métier sans recréer son identité, sa mémoire, son rattachement ni son environnement de travail.</p><div className="mt-8 grid gap-6 border-y border-[#1C1A17]/15 py-8 lg:grid-cols-[1fr_auto_1fr_auto_1fr]"><Proof title="Avant">Lucas<br/>Responsable du développement commercial<br/><small>Email · Calendrier · Mémoire · Environnement</small></Proof><b className="self-center text-3xl text-[#D10E63]">+</b><Proof title="Profil ajouté">Responsable relation client<br/><small>Qualification · Suivi · Synthèse</small></Proof><b className="self-center text-3xl text-[#D10E63]">=</b><Proof title="Après">Lucas<br/>Deux responsabilités<br/><small>Même identité · Expérience enrichie</small></Proof></div><p className="mt-6 font-semibold">Un profil métier change ce que Lucas peut prendre en charge. Il ne remplace pas celui qui travaille.</p></div></section>}
