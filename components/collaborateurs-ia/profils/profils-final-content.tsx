...
function CreateCard() { return <article className="flex min-h-[290px] flex-col rounded-2xl bg-[#151310] p-6 text-[#FAF8F3]"><p className="label text-[#F2A4C5]">Co-création</p><h3 className="mt-3 font-sf text-2xl font-bold">Votre responsabilité n’est pas encore dans le catalogue ?</h3><p className="mt-4 text-sm leading-7 text-[#CFC6B8]">Alma vous aide à interviewer les personnes concernées, formaliser leur méthode et préparer un profil métier testable.</p><p className="mt-4 text-sm">Alma · Coordinatrice de missions</p><div className="mt-auto flex flex-wrap gap-4 pt-6"><Link href="/decouvrir?intention=nouveau-profil-metier" className="rounded-full bg-[#D10E63] px-5 py-2.5 text-sm font-bold">Créer avec Alma →</Link><Link href="/co-createur-ia" className="py-2.5 text-sm font-bold underline">Devenir Co-créateur IA</Link></div></article> }
function Empty({ query }: { query: string }) { return <div role="status" className="mt-6 rounded-2xl bg-[#151310] p-7 text-[#FAF8F3]"><h3 className="font-sf text-2xl font-bold">Aucun profil ne correspond exactement à « {query} ».</h3><p className="mt-3 text-[#CFC6B8]">Alma peut vous aider à formaliser cette responsabilité.</p><Link href={`/decouvrir?intention=nouveau-profil-metier&q=${encodeURIComponent(query)}`} className="mt-5 inline-flex rounded-full bg-[#D10E63] px-5 py-3 text-sm font-bold">Créer avec Alma →</Link></div> }
