...
import { CREATOR_LABELS, DOMAIN_LABELS, STORE_ITEMS, type StoreItem } from '@/lib/store-catalog'
...
function ProfileCard({ profile, lang }: { profile: StoreItem; lang: 'fr' | 'en' }) { return <article className="flex min-h-[290px] flex-col rounded-2xl border border-[#DED6C8] bg-[#FAF8F3] p-5"><Link href={`/collaborateurs-ia/profils-metier?categorie=${profile.facet}`} className="label w-fit hover:text-[#D10E63]">{DOMAIN_LABELS[profile.facet]?.[lang] ?? profile.facet}</Link><h3 className="mt-3 font-sf text-xl font-bold">{profile.labels?.neutral?.[lang] ?? profile.name[lang]}</h3><p className="mt-3 text-sm leading-6 text-[#4E483F]">{profile.description[lang]}</p><p className="label mt-5">Compétences</p><p className="mt-2 text-xs leading-5 text-[#6E665A]">{profile.knowHow?.slice(0,3).map(item => item[lang]).join(' · ')}</p><p className="mt-auto pt-5 text-xs font-semibold">Créé par {CREATOR_LABELS[profile.creator][lang]}</p><Link href={`/decouvrir?store=${profile.slug}`} className="mt-4 inline-flex min-h-11 items-center justify-center rounded-full bg-[#1C1A17] px-5 text-sm font-bold text-white">{lang === 'fr' ? 'Ajouter à un Collaborateur IA' : 'Add to an AI Collaborator'} →</Link></article> }
function CreateCard() { return <article className="flex min-h-[290px] flex-col rounded-2xl bg-[#151310] p-6 text-[#FAF8F3]"><p className="label text-[#F2A4C5]">Co-création</p><h3 className="mt-3 font-sf text-2xl font-bold">Votre responsabilité n’est pas encore dans le catalogue ?</h3><p className="mt-4 text-sm leading-7 text-[#CFC6B8]"><AlmaInline /> Alma vous aide à interviewer les personnes concernées, formaliser leur méthode et préparer un profil métier testable.</p><p className="mt-4 text-sm"><AlmaInline /> Alma · Coordinatrice de missions IA</p><div className="mt-auto flex flex-wrap gap-4 pt-6"><Link href="/decouvrir?intention=nouveau-profil-metier" className="rounded-full bg-[#D10E63] px-5 py-2.5 text-sm font-bold">Créer avec Alma →</Link><Link href="/co-createur-ia" className="py-2.5 text-sm font-bold underline">Devenir Co-créateur IA</Link></div></article> }
