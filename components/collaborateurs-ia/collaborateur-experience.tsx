      ...
      <section aria-labelledby="collaborateurs-ia-title" className="hero-viewport relative flex items-center overflow-hidden border-b border-[#CFC5B5] bg-[#F3EFE6] pb-12 pt-24 sm:pb-14 sm:pt-28 [@media(min-width:1024px)_and_(max-height:850px)]:pb-10 [@media(min-width:1024px)_and_(max-height:850px)]:pt-24">
        ...
        <div className="editorial-shell relative grid items-center gap-9 sm:gap-12 lg:grid-cols-[1.02fr_.98fr] lg:gap-16 [@media(min-width:1024px)_and_(max-height:850px)]:gap-10">
            <div className="max-w-2xl lg:pr-2">
              ...
              <h1 id="collaborateurs-ia-title" className="text-balance text-center font-sf text-[clamp(1.9rem,4.2vw,3.5rem)] font-semibold leading-[1.05] tracking-[-.05em] text-[#1C1A17] sm:text-left [@media(min-width:1024px)_and_(max-height:850px)]:text-[clamp(2rem,3.4vw,3rem)]">{t.heroTitle}<span className="block text-[#D10E63]">{t.heroAccent}</span></h1>
              <p className="mx-auto mt-5 max-w-xl text-balance text-center text-[17px] font-semibold leading-7 text-[#302C27] sm:mx-0 sm:text-left md:text-lg md:leading-8">{t.heroLead}</p>
              <p className="mx-auto mt-3 max-w-xl text-balance text-center text-sm leading-6 text-[#625B50] sm:mx-0 sm:text-left md:text-[15px] md:leading-7">{t.heroBody}</p>
              <div className="mt-7 flex flex-col items-stretch gap-3 sm:flex-row sm:items-start [@media(min-width:1024px)_and_(max-height:850px)]:mt-6">
                <Link href="/decouvrir?source=collaborateurs-ia-hero" className="inline-flex min-h-13 items-center justify-center gap-2 rounded-full bg-[#D10E63] px-7 text-center text-sm font-bold text-white shadow-[0_16px_36px_-22px_rgba(209,14,99,.8)] outline-none transition hover:-translate-y-0.5 hover:bg-[#B90C58] focus-visible:ring-2 focus-visible:ring-[#D10E63] focus-visible:ring-offset-2">{t.start}<ArrowRight className="size-4 shrink-0" /></Link>
                <a href="#fonctionnement" className="inline-flex min-h-13 items-center justify-center gap-2 rounded-full border border-[#BFB4A4] bg-[#FAF8F3]/75 px-7 text-center text-sm font-bold outline-none transition hover:border-[#191715] hover:bg-[#191715] hover:text-white focus-visible:ring-2 focus-visible:ring-[#D10E63]">{t.understand}<ArrowDown className="size-4 shrink-0" /></a>
              </div>
            ...
            <aside aria-label={t.heroCardTitle} className="group relative mx-auto w-full max-w-md">
              ...
              <div className="relative w-full overflow-hidden rounded-[1.5rem] border border-white/10 bg-[#17130F] text-white shadow-[0_30px_80px_-20px_rgba(0,0,0,.65)] transition-transform duration-500 group-hover:-translate-y-1.5 sm:rounded-[1.75rem]">
                <div className="border-b border-white/10 px-6 py-5 sm:px-7"><p className="font-mono text-[9px] font-black uppercase tracking-[.17em] text-[#F2A4C5]">{t.heroCardKicker}</p><h2 className="mt-2 text-xl font-semibold tracking-[-.035em]">{t.heroCardTitle}</h2></div>
                <div className="p-5 sm:p-7 [@media(min-width:1024px)_and_(max-height:850px)]:p-5">
                  <div className="flex min-w-0 items-center">
                    ...
                    <div className="ml-3 min-w-0 sm:ml-4"><p className="text-sm font-bold">{t.heroCardTeam}</p><p className="mt-1 text-xs leading-5 text-[#AFA397]">{t.heroCardTeamBody}</p></div>
                  </div>
                  <dl className="mt-7 overflow-hidden rounded-2xl border border-white/10 bg-[#211E1B]">
                    {t.heroGroups.map((group, index) => <div key={group.label} className={`grid gap-2 p-4 sm:grid-cols-[8.5rem_1fr] sm:items-start sm:gap-3 [@media(min-width:1024px)_and_(max-height:850px)]:py-3 ${index > 0 ? 'border-t border-white/10' : ''}`}><dt className="font-mono text-[9px] font-black uppercase tracking-[.15em] text-[#F2A4C5]">{group.label}</dt><dd className="text-[12px] font-semibold leading-5 text-[#E7E0D5]">{group.value}</dd></div>)}
                  </dl>
...
const COPY = {
  fr: {
    eyebrow: 'Collaborateurs IA', heroTitle: 'Un Collaborateur IA rejoint votre équipe.', heroAccent: 'Ses capacités évoluent avec votre entreprise.', heroLead: 'Une identité, une mémoire et un rôle définis par votre entreprise.', heroBody: 'Il travaille avec vos équipes dans un espace partagé et utilise uniquement les informations, les outils et les actions que vous lui attribuez.',
    heroGroups: [{ label: 'Dans votre organisation', value: 'Rattaché à votre équipe · Mémoire sous votre contrôle' }, { label: 'Équipé pour ses missions', value: 'Profils métier · Compétences · Connexion à plus de 3 000 apps' }, { label: 'Infrastructure dédiée', value: 'Accès aux meilleurs modèles d’IA · Un serveur privé par Collaborateur' }], heroCardKicker: 'Votre organisation', heroCardTitle: 'Une équipe humaine et IA', heroCardTeam: 'Vos Collaborateurs IA', heroCardTeamBody: 'Chacun garde son identité, sa mémoire et ses responsabilités', heroCardRule: 'Vous définissez leurs responsabilités, leurs accès et les décisions qui restent humaines.', start: 'Commencer avec Alma', understand: 'Comprendre le Collaborateur IA', reassurance: ['Première mission gratuite', 'Sans carte bancaire'], almaGuidance: 'Personnalisation guidée par Alma',
  ...
  en: {
    eyebrow: 'AI Collaborators', heroTitle: 'An AI Collaborator joins your team.', heroAccent: 'Its capabilities evolve with your organization.', heroLead: 'An identity, memory and role defined by your organization.', heroBody: 'It works with your teams in a shared space and only uses the information, tools and actions you assign to it.',
    heroGroups: [{ label: 'In your organization', value: 'Placed with your team · Memory under your control' }, { label: 'Equipped for its missions', value: 'Job profiles · Skills · Connects to 3,000+ apps' }, { label: 'Dedicated infrastructure', value: 'Access to the best AI models · One private server per Collaborator' }], heroCardKicker: 'Your organization', heroCardTitle: 'A human and AI team', heroCardTeam: 'Your AI Collaborators', heroCardTeamBody: 'Each keeps its identity, memory and responsibilities', heroCardRule: 'You define their responsibilities, access and the decisions that remain human.', start: 'Start with Alma', understand: 'Understand the AI Collaborator', reassurance: ['First mission free', 'No card'], almaGuidance: 'Personalized with Alma',
