              ...
              <div className="relative w-full overflow-hidden rounded-[1.75rem] border border-white/10 bg-[#17130F] text-white shadow-[0_30px_80px_-20px_rgba(0,0,0,.65)] transition-transform duration-500 group-hover:-translate-y-1.5">
                ...
                <div className="p-6 sm:p-7">
                  ...
                  <dl className="mt-7 overflow-hidden rounded-2xl border border-white/10 bg-[#211E1B]">
                    {t.heroGroups.map((group, index) => <div key={group.label} className={`grid gap-3 p-4 sm:grid-cols-[8.5rem_1fr] sm:items-start sm:p-5 ${index > 0 ? 'border-t border-white/10' : ''}`}><dt className="font-mono text-[9px] font-black uppercase tracking-[.15em] text-[#F2A4C5]">{group.label}</dt><dd className="text-[12px] font-semibold leading-5 text-[#E7E0D5]">{group.value}</dd></div>)}
                  </dl>
