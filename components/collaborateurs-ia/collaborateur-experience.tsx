    ...
    <main className="overflow-hidden bg-[#F3EFE6] font-sf text-[#191715]">
      ...
      <section aria-labelledby="mission-proof-title" className="border-b border-[#CFC5B5] bg-[#191715] py-14 text-white sm:py-16 [@media(min-width:1024px)_and_(max-height:850px)]:py-12">
        <div className="editorial-shell">
          ...
          <div className="mt-6 flex flex-col gap-4 border-t border-white/10 pt-6 sm:flex-row sm:items-center sm:justify-between">
            ...
            <Link href={localizedHref('workspace', lang)} className="inline-flex min-h-11 shrink-0 items-center justify-center rounded-full border border-white/20 px-5 text-xs font-bold text-white transition hover:border-white hover:bg-white hover:text-[#191715]">{t.workspaceCta}<ArrowRight className="ml-2 size-3.5"/></Link>
          </div>
        </div>
      </section>

      <section aria-labelledby="governance-title" className="border-b border-[#CFC5B5] bg-[#FAF8F3] py-14 sm:py-20">
        <div className="editorial-shell">
          <SectionHeading id="governance-title" kicker={t.governanceKicker} title={t.governanceTitle} body={t.governanceBody} />
          <ol className="mt-8 grid overflow-hidden rounded-[20px] border border-[#CFC5B5] bg-[#EAE3D4] sm:mt-10 sm:grid-cols-2 lg:grid-cols-5">
            {t.governanceSteps.map((step, index) => <li key={step.title} className={`bg-[#FAF8F3] p-5 sm:p-6 ${index > 0 ? 'border-t border-[#CFC5B5] sm:[&:nth-child(2)]:border-t-0 lg:border-l lg:border-t-0' : ''} ${index > 1 ? 'sm:border-t' : ''}`}><span className="font-mono text-[9px] font-black text-[#B00C54]">0{index + 1}</span><h3 className="mt-4 font-semibold">{step.title}</h3><p className="mt-2 text-xs leading-5 text-[#625B50]">{step.body}</p></li>)}
          </ol>
          <div className="mt-6"><Link href={localizedHref('security', lang)} className="inline-flex items-center text-sm font-bold text-[#B00C54] underline decoration-[#B00C54]/30 underline-offset-4">{t.securityCta}<ArrowRight className="ml-2 size-4"/></Link></div>
        </div>
      </section>
      ...
      <section aria-labelledby="competences-title" className="border-y border-[#CFC5B5] bg-[#FAF8F3] py-14 sm:py-20 [@media(min-width:1024px)_and_(max-height:850px)]:py-14">
        <div className="editorial-shell">
          ...
          <div className="mt-8"><Link href={localizedHref('skills', lang)} className="inline-flex min-h-12 items-center rounded-full bg-[#D10E63] px-6 text-sm font-bold text-white transition hover:bg-[#B90C58]">{t.skillsCta}<ArrowRight className="ml-2 size-4" /></Link></div>
        </div>
      </section>
      ...
      <section aria-labelledby="capital-title" className="border-y border-[#191715] bg-[#F3EFE6] py-14 sm:py-20 [@media(min-width:1024px)_and_(max-height:850px)]:py-14">
        <div className="editorial-shell">
