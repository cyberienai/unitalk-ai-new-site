...
export function CollaborateurExperience() {
  ...
  return (
    <main>
      <section className="px-5 pb-14 pt-24 sm:px-8 sm:pt-28">
        <div className="mx-auto grid max-w-[1200px] items-center gap-10 lg:grid-cols-[1fr_1.05fr]">
          <div>
            ...
            <Link href="/decouvrir" className="mt-7 inline-flex rounded-full bg-[#D10E63] px-7 py-3.5 text-sm font-bold text-white">{t.create} →</Link>
...
function LucasSetup({ lang }: { lang: 'fr' | 'en' }) {
  ...
  return (
    <div className="min-h-[390px] rounded-3xl border border-[#DED6C8] bg-[#FAF8F3] p-5 shadow-[0_28px_65px_-48px_rgba(28,26,23,0.5)] sm:p-6">
      <div className="flex items-start justify-between gap-4"><div><p className="font-sf text-xl font-bold">Lucas</p><p className="text-xs text-[#6E665A]">Collaborateur IA · Nature IA</p></div><button type="button" onClick={() => setPaused((v) => !v)} className="text-xs font-semibold text-[#B00C54]">{paused ? 'Reprendre' : 'Pause'}</button></div>
      <div className="mt-5 flex flex-wrap gap-1.5">{HERO_STEPS.map((label, i) => <button key={label} type="button" onClick={() => setStep(i)} aria-pressed={step === i} className={`rounded-full border px-2.5 py-1 text-[10px] font-semibold ${step === i ? 'border-[#D10E63] bg-[#D10E63] text-white' : 'border-[#DED6C8] text-[#6E665A]'}`}>{label}</button>)}</div>
      <div className="mt-5 border-t border-[#DED6C8] pt-4"><AnimatePresence mode="wait"><motion.dl key={step} initial={reduce ? false : { opacity: 0, y: 5 }} animate={{ opacity: 1, y: 0 }} className="space-y-3">{rows.map(([label, value]) => <Row key={label} label={label} value={value} />)}</motion.dl></AnimatePresence></div>
    </div>
  )
}

function LightSection({ kicker, title, body, children }: { kicker: string; title: string; body: string[]; children: React.ReactNode }) {
  return <section className="px-5 py-16 sm:px-8 sm:py-20"><div className="mx-auto max-w-[1200px]"><Eyebrow>{kicker}</Eyebrow><h2 className="mt-3 max-w-4xl font-sf text-[30px] font-bold tracking-[-0.03em] sm:text-[40px]">{title}</h2>{body.map((p) => <p key={p} className="mt-3 max-w-3xl text-[15px] leading-relaxed text-[#4E483F]">{p}</p>)}<div className="mt-8">{children}</div></div></section>
}
...
function PublicProfile({ lang }: { lang: 'fr' | 'en' }) {
  return <Proof title="unitalk.ai/lucas" subtitle="Lucas · Collaborateur IA · Nature IA explicite"><p className="text-sm font-semibold">{lang === 'fr' ? 'Je peux vous aider à :' : 'I can help you:'}</p><ul className="mt-3 space-y-2 text-sm text-[#4E483F]"><li>• {lang === 'fr' ? 'suivre une réclamation' : 'track a complaint'}</li><li>• {lang === 'fr' ? 'préparer un rendez-vous' : 'prepare a meeting'}</li><li>• {lang === 'fr' ? 'retrouver l’état d’un dossier' : 'find a case status'}</li></ul><div className="mt-5 flex flex-wrap gap-2 text-xs"><span>Email</span><span>Rendez-vous</span><span>Appeler</span></div></Proof>
}

function Organization({ lang }: { lang: 'fr' | 'en' }) {
  const [scope, setScope] = useState('team')
  return <div className="mt-8 grid gap-5 rounded-3xl border border-[#DED6C8] bg-[#FAF8F3] p-5 lg:grid-cols-[1fr_20rem]"><div><h3 className="font-sf text-xl font-bold">{lang === 'fr' ? 'Il sait où se trouve sa place dans l’entreprise.' : 'It knows its place in the company.'}</h3><div className="mt-5 border-l-2 border-[#DED6C8] pl-5"><p className="font-bold">Solvea</p><p className="mt-3 text-sm">Sophie Martin · Humain</p><motion.div layout className="mt-3 rounded-xl bg-white p-3 text-sm"><b>Lucas · IA</b><br />Relation client · {scope}</motion.div><p className="mt-3 text-sm">Emma · IA · Planification</p></div></div><fieldset><legend className="text-xs font-bold uppercase text-[#6E665A]">Rattacher Lucas à</legend>{['person','team','department','organization'].map((s) => <label key={s} className="mt-3 flex gap-2 text-sm"><input type="radio" name="scope" checked={scope === s} onChange={() => setScope(s)} />{s}</label>)}<p className="mt-5 text-sm font-semibold">Ses responsabilités peuvent évoluer. Son identité reste.</p><p className="mt-3 text-xs text-[#6E665A]">Identités, organisations et appartenances gérées avec Clerk.</p></fieldset></div>
}

function DarkEnvironment({ lang }: { lang: 'fr' | 'en' }) {
  return <section className="bg-[#1C1A17] px-5 py-16 text-[#F3EFE6] sm:px-8 sm:py-20"><div className="mx-auto max-w-[1200px]"><Eyebrow dark>Son propre environnement</Eyebrow><h2 className="mt-3 font-sf text-[30px] font-bold sm:text-[40px]">Son propre environnement. Les outils de votre entreprise.</h2><p className="mt-4 max-w-3xl text-sm leading-relaxed text-[#C9C0B5]">Chaque Collaborateur IA repose sur un agent Hermes isolé dans son propre environnement, avec des ressources non partagées.</p><div className="mt-8 grid gap-5 lg:grid-cols-2"><Proof dark title="Serveur privé de Lucas" subtitle="Actif">{[['Stockage','Dédié'],['Mémoire vive','RAM dédiée'],['Processeur','CPU dédié'],['Fichiers','Disponibles'],['Navigateur Internet','Autorisé'],['Exécution de code','Disponible'],['Tâches planifiées','Actives'],['Secrets','Protégés'],['Hermes','Open source']].map(([a,b]) => <Row key={a} label={a} value={b} dark />)}</Proof><div className="space-y-3"><Activity title="Code" body="analyse_clients.py · 842 dossiers analysés · 17 anomalies détectées" /><Activity title="Navigateur" body="Recherche autorisée · 12 sources consultées · Résultat documenté" /><Activity title="Planification" body="Relancer les dossiers sans réponse · Demain · 09:00" /></div></div><Applications /></div></section>
}

function Applications() {
  return <div className="mt-10"><h3 className="font-sf text-2xl font-bold">Plus de 3 000 applications dans lesquelles agir.</h3><p className="mt-3 max-w-3xl text-sm leading-relaxed text-[#C9C0B5]">Les serveurs MCP de Pipedream connectent les applications par OAuth. L’entreprise choisit les données consultables et les actions autorisées.</p><div className="mt-5 flex flex-wrap gap-2 text-sm text-[#E7DED3]">CRM · Email · Calendrier · Messageries · Documents · Stockage · Finance · Support · + Plus de 3 000 applications</div><div className="mt-5 grid gap-3 sm:grid-cols-3"><Activity title="Connexion OAuth" body="Validation humaine" /><Activity title="Serveurs MCP Pipedream" body="Actions disponibles" /><Activity title="Droits" body="Définis par l’organisation" /></div><Proof dark title="Email" subtitle="Permissions"><Row label="Lire les messages" value="Autorisé" dark /><Row label="Préparer une réponse" value="Autorisé" dark /><Row label="Envoyer" value="Validation requise" dark /><Row label="Supprimer" value="Interdit" dark /></Proof><p className="mt-5 font-semibold">Il n’accède pas à tout. Il accède à ce dont son travail a besoin.</p><Link href="/collaborateurs-ia/applications" className="mt-5 inline-flex text-sm font-bold text-[#F2A4C5]">Explorer les applications →</Link></div>
}

function GatewaySection({ lang }: { lang: 'fr' | 'en' }) {
  return <LightSection kicker="Unitalk AI Gateway" title="Il ne dépend pas d’un seul modèle." body={[`L’AI Gateway donne à Lucas accès aux modèles disponibles et autorisés : GPT, Claude, Gemini, DeepSeek et modèles privés.`, 'Le Collaborateur reste. Le modèle peut changer.']}><div className="grid gap-5 lg:grid-cols-2"><Proof title="AI Gateway · Lucas" subtitle="Texte · Images · Audio · Vidéo">{[['GPT','Actif'],['Claude','Actif'],['Gemini','Actif'],['DeepSeek','Actif'],['Modèles privés','Selon configuration'],['Paiement','Crédits · Clés API · Hybride']].map(([a,b]) => <Row key={a} label={a} value={b} />)}</Proof><Proof title="Mixture of Agents" subtitle="Selon la configuration et la mission"><p className="text-center text-sm">GPT　 Claude　 Gemini<br />↓<br /><b>Modèle de revue</b><br />Compare · vérifie · synthétise<br />↓<br /><b>Lucas · Résultat proposé</b></p><p className="mt-5 text-sm text-[#4E483F]">Plusieurs modèles proposent. Un modèle de revue vérifie. Le Collaborateur IA restitue.</p></Proof></div><Link href="/ai-gateway" className="mt-5 inline-flex text-sm font-bold text-[#D10E63]">Découvrir AI Gateway →</Link></LightSection>
}

function MemorySection({ lang }: { lang: 'fr' | 'en' }) {
  const [saved, setSaved] = useState(false)
  return <LightSection kicker="Mémoire gouvernée" title="Il partage ce que votre entreprise sait. Il conserve ce que son travail lui apprend." body={['Lucas accède au contexte partagé autorisé par Solvea. Son expérience propre conserve uniquement les éléments validés.']}><div className="grid gap-4 lg:grid-cols-2"><Proof title="Contexte de Solvea" subtitle="Partagé selon les droits"><p className="text-sm leading-7">Produits et services<br />Clients<br />Procédures<br />Documents<br />Décisions<br />Terminologie</p></Proof><Proof title="Expérience de Lucas" subtitle="Gouvernée"><p className="text-sm leading-7">Dossiers suivis<br />Préférences confirmées<br />Méthodes éprouvées<br />Corrections validées<br />Contexte de ses responsabilités</p>{saved && <p className="mt-3 rounded-lg bg-[#E8F5EC] p-2 text-xs text-[#257A43]">Claire préfère être rappelée avant 10 h · Validée</p>}</Proof></div>{!saved && <div className="mt-4 rounded-2xl border border-[#DED6C8] bg-white p-4"><p className="text-xs font-bold uppercase text-[#6E665A]">Lucas propose de mémoriser</p><p className="mt-2 text-sm">Claire préfère être rappelée avant 10 h.</p><div className="mt-3 flex flex-wrap gap-2"><button className="rounded-full border px-3 py-1 text-xs">Ne pas mémoriser</button><button className="rounded-full border px-3 py-1 text-xs">Corriger</button><button onClick={() => setSaved(true)} className="rounded-full bg-[#D10E63] px-3 py-1 text-xs text-white">Conserver</button></div></div>}</LightSection>
}
...
function HermesBand({ lang }: { lang: 'fr' | 'en' }) {
  return <section className="bg-[#1C1A17] px-5 py-12 text-[#F3EFE6] sm:px-8"><div className="mx-auto max-w-[1200px]"><h2 className="font-sf text-[28px] font-bold sm:text-[36px]">Hermes lui donne sa capacité d’action. Unitalk lui donne sa place dans l’entreprise.</h2><div className="mt-6 grid gap-6 text-sm text-[#C9C0B5] md:grid-cols-2"><p><b className="text-white">Hermes</b><br />Agent autonome open source · Code · Navigation · Tâches planifiées · Mémoire · Outils</p><p><b className="text-white">Unitalk</b><br />Identité · Communication · Organisation · Serveur privé · AI Gateway · Applications OAuth et MCP · Gouvernance</p></div><Link href="/agent-hermes" className="mt-5 inline-flex text-sm font-bold text-[#F2A4C5]">Découvrir Hermes →</Link></div></section>
}
