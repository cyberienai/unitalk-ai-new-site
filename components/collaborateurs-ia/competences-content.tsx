...
import { SKILL_CATEGORY_LABELS, STORE_ITEMS, searchStore, storeItemHref, type Creator, type StoreItem } from '@/lib/store-catalog'
...
export function CompetencesContent() {
  ...
  return <main className="bg-[#F3EFE6] font-sf text-[#1C1A17]">
    ...
    <section className="px-5 py-16 sm:px-8"><div className="editorial-shell rounded-[18px] border border-[#DED6C8] bg-[#EAE3D4] p-7 sm:p-10"><p className="font-mono text-[10px] font-bold uppercase tracking-[.18em] text-[#B00C54]">Transmettre un savoir-faire</p><div className="mt-5 grid gap-8 lg:grid-cols-[1fr_auto] lg:items-end"><div><h2 className="max-w-2xl text-[34px] font-semibold leading-[1.06] tracking-[-.04em] sm:text-[44px]">Cette méthode n’existe pas encore dans le catalogue ?</h2><p className="mt-5 max-w-2xl text-[16px] leading-7 text-[#4E483F]">Alma vous aide à cadrer le besoin. Un Co-créateur IA peut ensuite interviewer les personnes concernées, formaliser la méthode, la tester et la versionner.</p><p className="mt-5 text-sm font-semibold">Alma · Coordinatrice de missions</p></div><div className="flex flex-col items-start gap-4 lg:items-end"><Link href="/inscription?source=competence-store&intention=nouvelle-competence" className="inline-flex min-h-12 items-center rounded-full bg-[#D10E63] px-6 text-sm font-bold text-white">Créer une compétence avec Alma →</Link><Link href="/co-createur-ia" className="text-sm font-bold text-[#4E483F] underline decoration-[#D10E63]/30 underline-offset-4">Devenir Co-créateur IA</Link></div></div></div></section>
  </main>
}
...
function SkillCard({ item, lang }: { item: StoreItem; lang: 'fr' | 'en' }) {
  ...
  return <article className="group relative flex min-h-[260px] flex-col rounded-[18px] border border-[#DED6C8] bg-[#F3EFE6] p-6 transition-colors hover:border-[#D10E63]/40"><p className="font-mono text-[9px] font-bold uppercase tracking-[.16em] text-[#B00C54]">{shortCategory(item.facet)}</p><h3 className="mt-4 text-xl font-semibold leading-7 tracking-[-.02em]">{content.name}</h3><p className="mt-3 line-clamp-3 text-sm leading-6 text-[#4E483F]">{content.description}</p><div className="mt-auto pt-6"><p className="font-mono text-[9px] font-bold uppercase tracking-[.14em] text-[#857C6E]">Entrée → Résultat</p><p className="mt-2 text-xs font-semibold leading-5">{content.input} → {content.result}</p><div className="mt-5 flex items-center justify-between border-t border-[#DED6C8] pt-4 text-xs text-[#6E665A]"><span>{item.creator === 'unitalk' ? 'Unitalk' : 'Communauté'}{item.version ? ` · v${item.version}` : ''}</span><ArrowRight className="size-4 text-[#D10E63] transition-transform group-hover:translate-x-1" /></div></div><Link href={storeItemHref(item)} aria-label={`Voir la compétence ${content.name}`} className="absolute inset-0 rounded-[18px] outline-none focus-visible:ring-2 focus-visible:ring-[#D10E63] focus-visible:ring-offset-2" /></article>
}
