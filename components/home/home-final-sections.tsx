...
export function HomeEvolution({ lang }: { lang: Lang }) {
  ...
  return <section className="bg-[#F3EFE6] px-5 py-16 sm:px-8 sm:py-20"><div className="editorial-shell"><div className="grid gap-10 lg:grid-cols-[.8fr_1.2fr] lg:items-center"><div><Kicker>{t.evolutionKicker}</Kicker><h2 className="mt-5 text-[clamp(2.7rem,5vw,5.2rem)] font-semibold leading-[.92] tracking-[-.065em]">{t.evolutionTitle}</h2><p className="mt-6 max-w-xl text-[17px] leading-8 text-[#4E483F]">{t.evolutionLead}</p><div className="mt-6 max-w-xl border-l-2 border-[#D10E63] pl-5"><h3 className="text-base font-semibold">{t.memoryTitle}</h3><p className="mt-2 text-sm leading-6 text-[#625B50]">{t.memoryBody}</p></div><Link href="/marketplace" className="mt-7 inline-flex items-center gap-2 text-sm font-bold text-[#B00C54]">{t.marketplaceCta}<ArrowRight className="size-4" /></Link></div><article className="relative overflow-hidden rounded-[2rem] bg-[#181615] p-6 text-white sm:p-8"><div aria-hidden className="absolute -right-24 -top-24 size-72 rounded-full bg-[#D10E63]/20 blur-3xl"/><div className="relative flex items-center gap-4 border-b border-white/10 pb-6"><Image src="/images/emma-avatar.png" alt="Emma" width={72} height={72} className="size-[72px] rounded-full object-cover ring-2 ring-[#F2A4C5]/35"/><div><p className="font-mono text-[9px] font-black uppercase tracking-[.16em] text-[#F2A4C5]">{identity}</p><h3 className="mt-2 text-2xl font-semibold">Emma</h3><p className="mt-1 text-xs font-semibold text-[#7DDBA1]">{role}</p></div></div><div className="relative mt-6"><div aria-hidden className="absolute bottom-8 left-[17px] top-8 w-px bg-gradient-to-b from-[#F2A4C5] via-[#D10E63] to-[#D10E63]/20"/><ol className="space-y-3">{capabilities.map(([label, value]) => <li key={label} className="relative grid grid-cols-[36px_1fr] items-center gap-3"><span className="z-10 flex size-9 items-center justify-center rounded-full border border-[#F2A4C5]/25 bg-[#2A2226] font-mono text-[10px] font-black text-[#F2A4C5]">+</span><div className="rounded-2xl border border-white/10 bg-white/[.045] px-4 py-3"><span className="block font-mono text-[8px] font-bold uppercase tracking-[.14em] text-[#8F877A]">{label}</span><strong className="mt-1 block text-sm text-white">{value}</strong></div></li>)}</ol><div className="ml-12 mt-4 flex items-center justify-between gap-4 rounded-2xl bg-[#D10E63] px-5 py-4"><div><p className="font-mono text-[8px] font-black uppercase tracking-[.15em] text-white/60">{lang === 'fr' ? 'Résultat' : 'Result'}</p><p className="mt-1 text-sm font-bold">{result}</p></div><ArrowRight className="size-5 shrink-0" /></div></div><div className="relative mt-6 border-t border-white/10 pt-5"><p className="text-sm leading-7 text-[#CFC6B8]">{t.exampleBody}</p><p className="mt-4 text-xs font-semibold leading-6 text-[#F2A4C5]">{t.governanceBody}</p></div></article></div></div></section>
}
