...
export function HomeIntentDoors({ lang }: { lang: Lang }) {
  ...
  return <section aria-labelledby="popular-missions-title" className="border-b border-[#D8D0C2] bg-[#F3EFE6] py-14 sm:py-20"><div className="editorial-shell"><div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between"><div><Kicker>Missions</Kicker><h2 id="popular-missions-title" className="mt-5 max-w-4xl text-[clamp(2.35rem,4vw,3.5rem)] font-bold leading-[.98] tracking-[-.05em]">{lang === 'fr' ? 'Par quoi voulez-vous commencer ?' : 'Where would you like to start?'}</h2><p className="mt-5 max-w-2xl text-[16px] leading-7 text-[#4E483F]">{lang === 'fr' ? <>Prospection, relation client, finance, direction ou opérations : choisissez une mission et <span className="whitespace-nowrap"><AlmaInline className="mr-1 align-[-.2em]"/>Alma</span> l’adapte à votre façon de travailler.</> : <>Sales, customer relations, finance, leadership or operations: choose a mission and <span className="whitespace-nowrap"><AlmaInline className="mr-1 align-[-.2em]"/>Alma</span> adapts it to the way you work.</>}</p></div><Link href={localizedHref('missions', lang)} className="inline-flex w-fit shrink-0 items-center gap-2 text-sm font-bold text-[#B00C54] underline decoration-[#D10E63]/30 underline-offset-4">{lang === 'fr' ? 'Explorer toutes les missions' : 'Explore all missions'}<ArrowRight className="size-4"/></Link></div><div className="mt-10 grid gap-4 md:grid-cols-2 xl:grid-cols-3">{missions.map(mission => <HomeMissionCard key={mission.slug} mission={mission} lang={lang}/>)}</div></div></section>
}
