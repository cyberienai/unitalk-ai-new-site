    ...
    <section className="bg-[#F3EFE6] py-14 sm:py-20">
      <div className="editorial-shell grid items-center gap-12 lg:grid-cols-2 lg:gap-16">
        ...
        <div ref={ref}>
          ...
          <div className="mt-3 flex items-center justify-between sm:hidden"><button type="button" onClick={() => selectScenario(scenarioIndex - 1)} aria-label={lang === 'fr' ? 'Exemple précédent' : 'Previous example'} className="flex size-11 items-center justify-center rounded-full border border-[#D8D0C2] bg-[#FAF8F3]"><ChevronLeft className="size-4" /></button><span className="text-xs font-semibold text-[#625B50]">{scenarioIndex + 1} / {scenarios.length}</span><button type="button" onClick={() => selectScenario(scenarioIndex + 1)} aria-label={lang === 'fr' ? 'Exemple suivant' : 'Next example'} className="flex size-11 items-center justify-center rounded-full border border-[#D8D0C2] bg-[#FAF8F3]"><ChevronRight className="size-4" /></button></div>
        </div>
      </div>
    </section>
  )
