...
export function PricingConfigurator() {
  ...
  return <section id="configurateur" className="mx-auto max-w-[1120px] px-5 pb-20 sm:px-8"><div className="grid border-y border-[#1C1A17]/15 lg:grid-cols-[1fr_340px]"><div className="lg:border-r lg:border-[#1C1A17]/15">
    ...
    <ConfigRow n="04" title="Licence Co-créateur IA Unitalk"><Check checked={cocreator} onChange={setCocreator} label={fr?'Ajouter à mon offre':'Add to my offer'}/><p className="mt-3 text-sm font-semibold">{formatEuro(unitalkPricing.aiCocreator.monthlyPrice,lang)}/{fr?'mois':'month'}</p><p className="mt-3 text-sm leading-6 text-[#4E483F]">{fr?'Interviewez les équipes, formalisez leur savoir-faire et créez des profils métier, compétences et missions réutilisables.':'Interview teams, formalize their know-how and create reusable job profiles, skills and missions.'}</p><Link href="/co-createur-ia" className="mt-4 inline-flex text-sm font-bold text-[#B00C54]">{fr?'Découvrir la formation et la licence':'Discover the training and license'} →</Link></ConfigRow>
