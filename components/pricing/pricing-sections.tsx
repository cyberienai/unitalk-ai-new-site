...
const COPY = {
  fr: {
    kicker: 'Tarifs Unitalk', title: 'Composez votre équipe.', accent: 'Maîtrisez son coût.',
    lead: 'Choisissez un palier Workspace selon le nombre d’humains, puis ajoutez les Collaborateurs IA utiles à vos missions.',
    primary: 'Configurer mon tarif', exploreMissions: 'Parler de ma mission à Alma', priceAnchor: 'Workspace Solo gratuit. Chaque Collaborateur IA : 49 € HT/mois.',
    ...
    selectedProfile: 'Profil de départ', summaryTitle: 'Calcul du prix', summaryBody: 'Votre total se met à jour selon la taille du Workspace et le nombre de Collaborateurs IA.', workspaceLine: 'Licence Workspace', collaboratorsLine: 'Collaborateurs IA', tokensLine: 'DeepSeek V4 Flash hébergé en Europe', includedTokens: '5 millions de tokens/mois inclus', millionTokens: 'millions de tokens/mois inclus', phoneLine: 'Téléphone inclus', phoneMinutes: '60 min par Collaborateur IA/mois', freeCreditsLine: 'Crédits Workspace · autres modèles', setupLine: 'Mise en service', setupValue: 'Accompagnée par Alma', monthly: 'Total mensuel HT', annual: 'Total annuel HT', today: 'À payer aujourd’hui', tax: 'Prix HT',
    usageKicker: 'Consommation IA', usageTitle: 'Trois ressources distinctes.', usageLead: 'Le Workspace inclut des crédits pour les assistants privés ou partagés et les autres modèles. Séparément, chaque Collaborateur IA inclut 5 millions de tokens mensuels avec DeepSeek V4 Flash. Vous pouvez aussi connecter vos propres clés API et régler directement le fournisseur.',
    creditsTitle: 'Autres modèles · Crédits Workspace', creditsBody: 'Utilisez les crédits mensuels inclus dans votre palier Workspace. Si vous le décidez, vous pouvez acheter une recharge à partir de 25 € ; aucune recharge automatique n’est annoncée.', creditsCta: 'Acheter des crédits', creditsDetail: 'Voir le détail',
    ...
    migrationKicker: 'Déjà équipé ?', migrationTitle: 'Migrez votre agent Hermes ou OpenClaw vers Unitalk.', migrationBody: 'Commandez la reprise des éléments compatibles de votre configuration, de vos outils et de vos données disponibles dans des formats exportables. Unitalk permet de déployer des agents open source autonomes à l’échelle de l’entreprise.', migrationCta: 'Commander la migration',
    continue: 'Enregistrer et continuer', nextStep: 'Étape suivante : inscription ou confirmation de commande.', activationNote: '0 € aujourd’hui pour démarrer la mission offerte. La configuration payante n’est activée qu’après votre confirmation explicite.', error: 'La configuration n’a pas pu être enregistrée. Réessayez.',
  ...
  en: {
    kicker: 'Unitalk pricing', title: 'Build your team.', accent: 'Control its cost.',
    lead: 'Choose a Workspace tier based on the number of humans, then add the AI Collaborators your missions need.',
    primary: 'Configure my pricing', exploreMissions: 'Discuss my mission with Alma', priceAnchor: 'Free Solo Workspace. Each AI Collaborator: €49 excl. tax/month.',
    ...
    selectedProfile: 'Starting profile', summaryTitle: 'Price calculation', summaryBody: 'Your total updates with the Workspace size and number of AI Collaborators.', workspaceLine: 'Workspace license', collaboratorsLine: 'AI Collaborators', tokensLine: 'DeepSeek V4 Flash hosted in Europe', includedTokens: '5 million tokens/month included', millionTokens: 'million tokens/month included', phoneLine: 'Included phone usage', phoneMinutes: '60 min per AI Collaborator/month', freeCreditsLine: 'Workspace credits · other models', setupLine: 'Setup', setupValue: 'Guided by Alma', monthly: 'Monthly total excl. tax', annual: 'Annual total excl. tax', today: 'Due today', tax: 'Price excludes tax',
    usageKicker: 'AI usage', usageTitle: 'Three distinct resources.', usageLead: 'Workspace includes credits for private or shared assistants and other models. Separately, each AI Collaborator includes 5 million monthly tokens with DeepSeek V4 Flash. You can also connect your own API keys and pay the provider directly.',
    creditsTitle: 'Other models · Workspace credits', creditsBody: 'Use the monthly credits included with your Workspace tier. If you choose, you can buy a top-up from €25; no automatic top-up is advertised.', creditsCta: 'Buy credits', creditsDetail: 'View details',
    ...
    migrationKicker: 'Already equipped?', migrationTitle: 'Move your Hermes or OpenClaw agent to Unitalk.', migrationBody: 'Order the transfer of compatible parts of your configuration, tools and data available in exportable formats. Unitalk lets you deploy autonomous open-source agents across your organization.', migrationCta: 'Order migration',
    continue: 'Save and continue', nextStep: 'Next step: sign-up or order confirmation.', activationNote: '€0 today to start the included mission. Your paid configuration is activated only after your explicit confirmation.', error: 'We could not save this configuration. Please try again.',
...
export function PricingHero() {
  ...
  return (
    <section className="relative overflow-hidden border-b border-[#D8CEBE] bg-[#EAE3D4] px-5 pb-8 pt-24 sm:px-8 sm:pb-10 sm:pt-28">
      ...
      <div className="editorial-shell relative flex flex-col items-center text-center">
        <Kicker>{t.kicker}</Kicker><h1 className="mt-4 max-w-5xl text-balance text-[clamp(2.6rem,5.5vw,5rem)] font-semibold leading-[.9] tracking-[-.07em]">{t.title}<span className="block text-[#D10E63]">{t.accent}</span></h1>
        <p className="mt-6 max-w-2xl text-[15px] leading-7 text-[#4E483F] sm:text-[16px]">{withAlmaAvatar(t.lead)}</p><p className="mt-4 text-sm font-black text-[#1C1A17]">{t.priceAnchor}</p><div className="mt-5 flex w-full flex-col justify-center gap-3 sm:w-auto sm:flex-row sm:items-center"><Link href="#configurateur" className="inline-flex min-h-11 items-center justify-center rounded-full bg-[#D10E63] px-6 text-sm font-bold text-white transition hover:-translate-y-0.5 hover:bg-[#B00C54]">{t.primary}<ArrowRight className="ml-2 size-4"/></Link><Link href="/decouvrir?source=tarifs" className="inline-flex min-h-11 items-center justify-center px-3 text-sm font-bold text-[#4E483F] underline decoration-[#D10E63]/35 underline-offset-4 hover:text-[#B00C54]">{t.exploreMissions}</Link></div>
        <ul className="mt-7 flex flex-wrap justify-center gap-x-6 gap-y-2 border-t border-[#CFC5B5] pt-4 text-[11px] font-bold text-[#625B50]">{t.proofs.map(item => <li key={item} className="flex items-center gap-2"><Check className="size-3.5 text-[#D10E63]"/>{item}</li>)}</ul>
      </div>
    </section>
  )
}
      ...
      <section className="px-5 py-14 sm:px-8 sm:py-20">
        <div className="editorial-shell">
          ...
          <div className="mt-10 grid items-stretch gap-5 lg:grid-cols-3">
            ...
            <aside className="order-first flex h-full flex-col rounded-[26px] bg-[#211E1A] p-6 text-white shadow-[0_28px_70px_-38px_rgba(28,26,23,.8)] sm:p-7 lg:order-none"><p className="font-mono text-[10px] font-black uppercase tracking-[.16em] text-[#F2A4C5]">{billingPeriod === 'annual' ? t.annual : t.monthly}</p>{billingPeriod === 'annual' ? <><p className="mt-5 font-sf text-5xl font-semibold tracking-[-.065em]">{annualTotal} €<span className="ml-1 text-xs font-medium tracking-normal text-white/70"> HT/an</span></p><p className="mt-2 text-xs font-bold text-white/80">{t.equivalent} {formatPrice(effectiveMonthlyTotal, lang)} € HT/mois · {t.annualOffer}</p></> : <p className="mt-5 font-sf text-5xl font-semibold tracking-[-.065em]">{monthlyTotal} €<span className="ml-1 text-xs font-medium tracking-normal text-white/70"> HT/mois</span></p>}<dl className="mt-6 overflow-hidden rounded-2xl border border-white/15 bg-white/[.07]"><WorkspaceSummary label={t.workspaceLine} name={selectedTier.name} detail={`${selectedTier.users}${organizationPrice ? billingPeriod === 'annual' ? ` · ${organizationPrice * 10} € HT/an` : ` · ${organizationPrice} € HT/mois` : ''}`}/><Summary label={t.freeCreditsLine} value={freeCreditsValue} dark/>{collaborators > 0 && <><Summary label={t.collaboratorsLine} value={billingPeriod === 'annual' ? `${collaborators} × 490 € HT/an` : `${collaborators} × 49 € HT/mois`} dark/><Summary label={t.tokensLine} value={includedTokensLabel} dark/><Summary label={t.phoneLine} value={t.phoneMinutes} dark/></>}<Summary label={t.setupLine} value={<span className="inline-flex items-center gap-1.5"><AlmaFace em={1.2}/>{t.setupValue}</span>} dark/></dl><p className="mt-5 text-sm font-semibold text-white/85">{t.today} : 0 €</p><p className="mt-2 text-xs leading-5 text-white/65">{t.nextStep}</p><button type="button" onClick={submit} disabled={pending} aria-busy={pending} className="mt-5 inline-flex min-h-12 w-full items-center justify-center rounded-full bg-[#D10E63] px-6 text-sm font-bold text-white transition hover:bg-[#B00C54] disabled:opacity-60">{t.continue}<ArrowRight className="ml-2 size-4"/></button>{error && <p role="alert" className="mt-4 text-xs text-white">{error}</p>}</aside>
          </div>
