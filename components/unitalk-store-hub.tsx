...
function itemsForCategory(categoryId: string, lang: Lang): MarketplaceItem[] {
  ...
  if (storeType) {
    ...
    return items.map((item) => ({
      ...
      highlights: (item.knowHow ?? item.enables ?? item.produces)?.slice(0, 2).map((value) => value[lang]),
      ...
      <section id="categories" className="scroll-mt-40 px-5 pb-24 pt-10 sm:px-8 sm:pt-12 lg:pb-32">
        <div className="mx-auto w-full max-w-6xl">
            <div id="marketplace-results" role={collaboratorsOnly ? undefined : 'tabpanel'} aria-labelledby={collaboratorsOnly ? undefined : `marketplace-tab-${activeCategory.id}`} className="scroll-mt-[184px]">
                 {collaboratorsOnly && <div className="mb-7"><h2 className="text-3xl font-semibold tracking-[-.04em] sm:text-4xl">{lang === 'fr' ? 'Les Collaborateurs IA de référence' : 'Reference AI Collaborators'}</h2><p className="mt-3 max-w-3xl text-sm leading-6 text-[#625B50]">{lang === 'fr' ? 'Choisissez une identité de départ. Alma vous aide à cadrer chaque mission avant toute action.' : 'Choose a starting identity. Alma helps you scope each mission before any action.'}</p></div>}
  ...
  if (hasDirectAdd) {
    return (
      <Link href={item.addHref!} aria-label={`${labels.addProfile} : ${item.title}`} className="group relative flex min-h-[260px] flex-col overflow-hidden rounded-[16px] border border-[#D8D0C2] bg-[#FBF9F4] p-6 text-left outline-none transition-[transform,border-color,background-color,box-shadow] duration-300 hover:-translate-y-0.5 hover:border-[var(--profile-accent)] hover:bg-[#FFFDF9] hover:shadow-[0_18px_45px_-38px_rgba(28,26,23,.8)] focus-visible:border-[var(--profile-accent)] focus-visible:ring-2 focus-visible:ring-[var(--profile-accent)] focus-visible:ring-offset-2" style={style}>
        ...
        {category.id === 'competences' && <dl className="mt-4 grid gap-2 rounded-xl bg-[#F0EBE1] p-3 text-xs"><div><dt className="font-mono text-[9px] font-black uppercase tracking-[.12em] text-[#857C6E]">{lang === 'fr' ? 'Contexte d’entrée' : 'Input context'}</dt><dd className="mt-1 font-semibold leading-5 text-[#3F3A33]">{item.input ?? (lang === 'fr' ? 'Contexte précisé avec Alma' : 'Context scoped with Alma')}</dd></div><div className="border-t border-[#D8D0C2] pt-2"><dt className="font-mono text-[9px] font-black uppercase tracking-[.12em] text-[#857C6E]">{lang === 'fr' ? 'Résultat produit' : 'Produced result'}</dt><dd className="mt-1 font-semibold leading-5 text-[#3F3A33]">{item.result ?? (lang === 'fr' ? 'Résultat documenté à valider' : 'Documented result to approve')}</dd></div></dl>}
        {category.id === 'competences' && item.profileKeys && item.profileKeys.length > 0 && <div className="mt-4"><p className="font-mono text-[9px] font-black uppercase tracking-[.12em] text-[#857C6E]">{lang === 'fr' ? 'Profils compatibles' : 'Compatible profiles'}</p><div className="mt-2 flex flex-wrap gap-1.5">{item.profileKeys.slice(0, 2).map((slug) => <span key={slug} className="rounded-full border border-[#D8D0C2] px-2.5 py-1 text-[10px] font-semibold text-[#4E483F]">{PROFILE_NAMES.get(slug)?.[lang] ?? slug}</span>)}</div></div>}
        {category.id === 'competences' && <p className="mt-4 text-[10px] font-semibold leading-4 text-[#766D61]">{item.proof}</p>}
