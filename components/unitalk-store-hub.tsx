...
const COPY = {
  fr: {
    ...
    allDepartments: 'Tous les départements', result: 'résultat', results: 'résultats', almaTitle: 'Vous hésitez entre plusieurs Collaborateurs IA ?', almaBody: 'Décrivez la mission à Alma. Elle vous aide à choisir une identité et à identifier les profils métier et compétences nécessaires.', almaAction: 'Décrire ma mission à Alma', finalTitle: 'Vous ne choisissez pas un métier définitif.', finalBody: 'Vous choisissez une identité qui conserve sa mémoire et évolue avec les profils métier et les compétences dont votre entreprise a besoin.', finalProfiles: 'Explorer les profils métier',
    heroProofs: ['Même identité', 'Mémoire conservée', 'Profils métier évolutifs', 'Accès sous votre contrôle'], starterMission: 'Mission de départ', exampleResult: 'Exemple de résultat', ctaHelp: 'Alma vous aide d’abord à cadrer la mission.', compatibleProfiles: 'Voir les profils métier',
  },
  en: {
    ...
    <main className="min-h-screen overflow-x-clip bg-[#F3EFE6] font-sf text-[#1C1A17]">
      <section className={`bg-[#EAE3D4] px-5 sm:px-8 ${collaboratorsOnly ? 'pb-8 pt-20 sm:pb-10 sm:pt-24' : 'pb-12 pt-24 sm:pb-14 sm:pt-28'}`}>
        <div className="mx-auto w-full max-w-6xl">
          <h1 className="max-w-6xl text-[clamp(2.8rem,5.7vw,5.25rem)] font-semibold leading-[.88] tracking-[-.07em] text-balance">{activeCategory.id === 'competences' ? (lang === 'fr' ? <>Un savoir-faire précis.<span className="text-[#6246B5] lg:block">Réutilisable mission après mission.</span></> : <>Precise know-how.<span className="text-[#6246B5] lg:block">Reusable across missions.</span></>) : <>{t.heroTitle.slice(0, -t.heroAccent.length)}<span className="text-[#D10E63] lg:block">{t.heroAccent}</span></>}</h1>
          <p className="mt-7 max-w-6xl text-[16px] leading-7 text-[#4E483F] text-pretty lg:pr-8">{activeCategory.id === 'competences' ? (lang === 'fr' ? 'Choisissez une méthode précise que votre Collaborateur IA peut appliquer dans un contexte défini, avec un résultat attendu et vos validations.' : 'Choose a precise method your AI Collaborator can apply in a defined context, with an expected result and your approvals.') : t.heroLead}</p>
      ...
      <section id="categories" className="scroll-mt-40 px-5 pb-24 pt-10 sm:px-8 sm:pt-12 lg:pb-32">
        <div className="mx-auto w-full max-w-6xl">
            <div id="marketplace-results" role={collaboratorsOnly ? undefined : 'tabpanel'} aria-labelledby={collaboratorsOnly ? undefined : `marketplace-tab-${activeCategory.id}`} className="scroll-mt-[184px]">
                  {collaboratorsOnly && <div className="mb-7"><h2 className="text-3xl font-semibold tracking-[-.04em] sm:text-4xl">{lang === 'fr' ? 'Choisissez votre Collaborateur IA' : 'Choose your AI Collaborator'}</h2><p className="mt-3 max-w-3xl text-sm leading-6 text-[#625B50]">{lang === 'fr' ? 'Alma vous aide à cadrer la mission avant la création de votre espace. Aucune action n’est lancée sans votre validation.' : 'Alma helps you scope the mission before your workspace is created. No action starts without your approval.'}</p><ul className="mt-3 flex flex-wrap gap-x-5 gap-y-2 text-xs font-bold text-[#625B50]"><li>{lang === 'fr' ? 'Première mission offerte' : 'First mission included'}</li><li>{lang === 'fr' ? 'Sans carte bancaire' : 'No credit card'}</li></ul></div>}
...
function MarketplaceItemCard({ item, lang, category, labels, featuredLast = false }: { item: MarketplaceItem; lang: Lang; category: Category; labels: { details: string; available: string; preparation: string; addProfile: string }; featuredLast?: boolean }) {
  ...
  if (hasDirectAdd) {
    return (
      <Link href={item.addHref!} aria-label={`${labels.addProfile} : ${item.title}`} className="group relative flex min-h-[260px] flex-col overflow-hidden rounded-[16px] border border-[#D8D0C2] bg-[#FBF9F4] p-6 text-left outline-none transition-[transform,border-color,background-color,box-shadow] duration-300 hover:-translate-y-0.5 hover:border-[var(--profile-accent)] hover:bg-[#FFFDF9] hover:shadow-[0_18px_45px_-38px_rgba(28,26,23,.8)] focus-visible:border-[var(--profile-accent)] focus-visible:ring-2 focus-visible:ring-[var(--profile-accent)] focus-visible:ring-offset-2" style={style}>
        ...
        <h3 className="mt-4 line-clamp-2 text-[23px] font-semibold leading-[1.08] tracking-[-.04em] text-[#1C1A17]">{item.title}</h3>
        <p className="mt-3 line-clamp-2 text-[13px] leading-6 text-[#625B50]">{item.description}</p>
        {category.id === 'competences' && <div className="mt-4 grid gap-3 sm:grid-cols-[1fr_auto] sm:items-end"><dl className="grid gap-2 rounded-xl bg-[#F0EBE1] p-3 text-xs"><div><dt className="font-mono text-[9px] font-black uppercase tracking-[.12em] text-[#857C6E]">{lang === 'fr' ? 'Contexte d’application' : 'Application context'}</dt><dd className="mt-1 line-clamp-1 font-semibold leading-5 text-[#3F3A33]">{item.input ?? (lang === 'fr' ? 'Contexte précisé avec Alma' : 'Context scoped with Alma')}</dd></div><div className="border-t border-[#D8D0C2] pt-2"><dt className="font-mono text-[9px] font-black uppercase tracking-[.12em] text-[#857C6E]">{lang === 'fr' ? 'Résultat produit' : 'Produced result'}</dt><dd className="mt-1 line-clamp-1 font-semibold leading-5 text-[#3F3A33]">{item.result ?? (lang === 'fr' ? 'Résultat documenté à valider' : 'Documented result to approve')}</dd></div></dl>{item.profileKeys && item.profileKeys.length > 0 && <div><p className="font-mono text-[9px] font-black uppercase tracking-[.12em] text-[#857C6E]">{lang === 'fr' ? 'Profils compatibles' : 'Compatible profiles'}</p><div className="mt-2 flex flex-wrap gap-1.5">{item.profileKeys.slice(0, 2).map((slug) => <span key={slug} className="rounded-full border border-[#D8D0C2] px-2.5 py-1 text-[10px] font-semibold text-[#4E483F]">{PROFILE_NAMES.get(slug)?.[lang] ?? slug}</span>)}</div></div>}</div>}
        {category.id === 'competences' && <p className="mt-2 text-[10px] font-semibold leading-4 text-[#766D61]">{item.proof}</p>}
        {item.highlights && item.highlights.length > 0 && <div className="mt-5"><p className="font-mono text-[9px] font-black uppercase tracking-[.14em] text-[#857C6E]">{item.highlightsLabel}</p><ul className="mt-2 space-y-1.5">{item.highlights.map((highlight) => <li key={highlight} className="flex gap-2 text-xs font-semibold leading-5 text-[#4E483F]"><span aria-hidden className="mt-2 size-1 shrink-0 rounded-full bg-[var(--profile-accent)]"/>{highlight}</li>)}</ul></div>}
        <div className="mt-auto pt-6">
          <div className="border-t border-[#DED6C8] pt-4 transition-colors group-hover:border-[var(--profile-accent)] group-focus-visible:border-[var(--profile-accent)]">
            ...
            <span className="mt-3 flex min-h-10 items-center justify-between rounded-full border border-[#CFC5B5] px-4 text-xs font-bold text-[#1C1A17] transition-[color,background-color,border-color] group-hover:border-[var(--profile-accent)] group-hover:bg-[var(--profile-accent)] group-hover:text-white group-focus-visible:border-[var(--profile-accent)] group-focus-visible:bg-[var(--profile-accent)] group-focus-visible:text-white sm:border-transparent sm:px-0 sm:group-hover:px-4 sm:group-focus-visible:px-4">
  ...
  const content = (
    <>
      <div>
        ...
        <h3 className="line-clamp-2 text-[26px] font-semibold leading-[1.04] tracking-[-.045em] text-[#1C1A17]">{item.title}</h3>
