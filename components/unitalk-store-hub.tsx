...
const PROFILE_DEPARTMENTS = [
  ...
  { id: 'donnees', label: { fr: 'Données', en: 'Data' }, profiles: ['analyste-donnees', 'responsable-informatique-decisionnelle', 'analyste-web'] },
  ...
  { id: 'cybersecurite-qualite-logicielle', label: { fr: 'Cybersécurité et qualité logicielle', en: 'Cybersecurity & software quality' }, profiles: ['responsable-cybersecurite', 'ingenieur-qualite-logicielle'] },
...
export function UnitalkStoreHub({ collaboratorsOnly = false, fixedLang, initialCategoryId }: { collaboratorsOnly?: boolean; fixedLang?: SiteLang; initialCategoryId?: string }) {
  ...
  const catalogIntro = activeCategory.id === 'applications'
  ...
  return (
    <div>
      <div className="mb-5 lg:hidden">
        <label className={category.id === 'competences' ? 'sr-only' : 'block font-mono text-[9px] font-black uppercase tracking-[.15em] text-[#766D61]'} htmlFor={`${category.id}-facet`}>{facetTitle}</label>
        <select id={`${category.id}-facet`} value={activeFacet} onChange={(event) => onFacet(event.target.value)} className={`${category.id === 'competences' ? '' : 'mt-2 '}h-12 w-full rounded-full border border-[#CFC5B5] bg-[#FAF8F3] px-4 text-sm font-bold outline-none focus:border-[var(--facet-accent)] focus:ring-2 focus:ring-[#1C1A17]/10`} style={{ '--facet-accent': category.accent } as CSSProperties}>
      ...
      <div className="grid gap-7 lg:grid-cols-[220px_minmax(0,1fr)] lg:items-start xl:grid-cols-[240px_minmax(0,1fr)]">
        <aside className="sticky top-[164px] hidden max-h-[calc(100vh-180px)] overflow-y-auto overscroll-contain rounded-[18px] border border-[#D8D0C2] bg-[#EAE3D4] p-3 lg:block">
          {category.id !== 'competences' && <p className="px-3 pb-3 pt-2 font-mono text-[9px] font-black uppercase tracking-[.16em] text-[#766D61]">{facetTitle}</p>}
