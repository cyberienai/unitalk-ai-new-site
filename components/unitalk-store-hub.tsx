        ...
        <div className="mx-auto w-full max-w-6xl">
             <div id="marketplace-results" role={collaboratorsOnly ? 'region' : 'tabpanel'} aria-labelledby={`marketplace-tab-${activeCategory.id}`} className="scroll-mt-[184px]">
                    ...
                    {!isCollaboratorsLanding && !isProfilesCategory && !usesCatalogSidebar && <div className="mb-5 sm:mb-6"><h2 className="text-[28px] font-semibold tracking-[-.04em] sm:text-[34px]">{activeCategory.title[lang]}</h2><p className="mt-2 max-w-4xl text-sm leading-6 text-[#625B50]">{activeCategory.description[lang]}</p>{activeCategory.id === 'serveurs-ia' && <Link href="/collaborateurs-ia/serveurs" className="mt-5 inline-flex min-h-10 items-center border-b border-[#216641] text-xs font-bold text-[#216641]">{lang === 'fr' ? 'Voir les options d’infrastructure' : 'View infrastructure options'}<span aria-hidden className="ml-2">→</span></Link>}</div>}
                      {isCollaboratorsLanding && <div className="mb-5 flex flex-col gap-3 sm:mb-7 lg:flex-row lg:items-end lg:justify-between"><div><h2 className="text-[25px] font-semibold leading-tight tracking-[-.04em] sm:text-[30px]">{lang === 'fr' ? 'Commencez par choisir son profil métier.' : 'Start by choosing their job profile.'}</h2><p className="mt-2 max-w-4xl text-[14px] font-medium leading-6 text-[#4E483F] sm:text-[15px] sm:leading-7">{lang === 'fr' ? 'Attribuez-lui ensuite un nom, un visage et une voix.' : 'Then give them a name, a face and a voice.'}</p></div><Link href={activeCategory.href} className="inline-flex min-h-11 w-fit shrink-0 items-center border-b border-[#857C6E] text-xs font-bold text-[#625B50] outline-none hover:text-[#1C1A17] focus-visible:ring-2 focus-visible:ring-[#D10E63]">{activeCategory.explain[lang]}<span aria-hidden="true" className="ml-3">↗</span></Link></div>}
