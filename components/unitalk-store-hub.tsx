        ...
        <div className="mx-auto w-full max-w-6xl">
            <div id="marketplace-results" role={collaboratorsOnly ? 'region' : 'tabpanel'} aria-labelledby={`marketplace-tab-${activeCategory.id}`} className="scroll-mt-[184px]">
                  {!isCollaboratorsLanding && <div className="mb-5 sm:mb-6"><h2 className="text-[28px] font-semibold tracking-[-.04em] sm:text-[34px]">{activeCategory.title[lang]}</h2><p className="mt-2 text-sm leading-6 text-[#625B50] xl:whitespace-nowrap">{activeCategory.description[lang]}</p></div>}
                   {isCollaboratorsLanding && <div className="mb-5 flex flex-col gap-3 sm:mb-7 lg:flex-row lg:items-end lg:justify-between"><div><h2 className="text-[26px] font-semibold leading-tight tracking-[-.04em] sm:text-[30px]">{lang === 'fr' ? 'Un Collaborateur IA pour chaque département' : 'An AI Collaborator for every department'}</h2><p className="mt-2 max-w-4xl text-[14px] font-medium leading-6 text-[#4E483F] sm:text-[15px] sm:leading-7">{activeCategory.description[lang]}</p></div><Link href={activeCategory.href} className="inline-flex min-h-11 w-fit shrink-0 items-center border-b border-[#857C6E] text-xs font-bold text-[#625B50] outline-none hover:text-[#1C1A17] focus-visible:ring-2 focus-visible:ring-[#D10E63]">{activeCategory.explain[lang]}<span aria-hidden="true" className="ml-3">↗</span></Link></div>}
