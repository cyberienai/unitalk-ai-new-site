  ...
  return (
    <div>
      ...
      {category.id === 'competences' && category.missing && <section className="mt-12 overflow-hidden rounded-[22px] bg-[#181615] text-white sm:mt-16 lg:grid lg:grid-cols-[1fr_auto] lg:items-center">
        ...
        <div className="flex flex-col gap-4 border-t border-white/10 p-7 lg:items-end lg:border-l lg:border-t-0 lg:p-9"><Link href={category.missing.href} className="inline-flex min-h-12 w-full items-center justify-center rounded-full bg-[#D10E63] px-6 text-sm font-bold text-white transition-colors hover:bg-[#B00C54] lg:w-auto">{lang === 'fr' ? 'Créer ma compétence' : 'Create my skill'}<span aria-hidden className="ml-2">→</span></Link><p className="text-center text-xs font-semibold text-[#AFA397]">{lang === 'fr' ? 'Accompagné par Alma' : 'Guided by Alma'}</p></div>
      </section>}
    </div>
  )
}
