...
export function AcmePublicProfile({ slug }: { slug: string }) {
  ...
  return (
    <main className="min-h-screen bg-[#F3EFE6] text-[#1C1A17]">
      ...
      <div className="mx-auto max-w-[1200px] px-5 py-5 sm:px-8"><nav className="text-sm text-[#6E665A]"><span>{ACME.name}</span><span className="mx-2">/</span><span className="text-[#1C1A17]">{member.name}</span></nav></div>
