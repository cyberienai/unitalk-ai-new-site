  ...
  return (
    ...
    >
      ...
      <div className="relative rounded-3xl border border-[rgba(255,255,255,0.08)] bg-[#111111] overflow-hidden">
        {/* Dots */}
        <div className="flex items-center justify-center gap-2 border-b border-[#222222] bg-[#0F0F0F] px-6 py-4">
          {SLIDES.map((_, idx) => (
            <button
              key={idx}
        ...
        <div className="relative h-96 overflow-hidden">
          <AnimatePresence mode="wait">
            <motion.div
              ...
              className="absolute inset-0 flex flex-col p-6 space-y-4"
            >
              {/* Title */}
              <h2 className="font-heading text-xl leading-tight text-white whitespace-pre-line">{slide.title}</h2>

              {/* Avatar with label */}
              <div className="flex items-center gap-3">
                <AlmaAvatar state={slide.avatarState as any} size={40} showGlow={true} />
                <span className="text-xs text-[#8E8E93]">{slide.avatarLabel}</span>
              </div>

              {/* Body text */}
              <p className="text-sm leading-relaxed text-[#8E8E93] whitespace-pre-line flex-1 overflow-y-auto">{slide.content}</p>
            </motion.div>
          </AnimatePresence>
        </div>

        {/* Footer with pricing */}
        <div className="border-t border-[#222222] bg-[#0F0F0F] px-6 py-4">
