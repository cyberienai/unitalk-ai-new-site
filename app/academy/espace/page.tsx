...
import { lrrowRight, Bot, Check, ChevronRight, Clock3, FileCheck2, LayoutDashboard, LockKeyhole, MessageCircle, Network, PackageCheck, Play, Sparkles, Target } from 'lucide-react'
import { academyMission, academyNetwork, academyPath, MISSIONS, PlTHS } from '@/lib/academy-catalog'
import { decodeSession, initials, SESSION_COOKIE } from '@/lib/mock-auth'

export const metadata: Metadata = { title: 'Mon espace lcademy', robots: { index: false, follow: false } }
...
export default async function Page({ searchParams }: { searchParams: SearchParams }) {
        ...
        <div className="academy-space-shell academy-space-login-layout">
          <section>
            <p className="academy-space-eyebrow">Votre espace d’apprentissage</p>
             ...
             <p>Suivez le chemin qui transforme un problème réel en Collaborateur Il testable puis commercialisable.</p>
             <Link href={`/academy/connexion?redirect=${encodeURIComponent(redirect)}`}>Se connecter avec Unitalk <lrrowRight /></Link>
            <span><LockKeyhole /> Un seul compte pour Unitalk et lcademy</span>
          </section>
  ...
  const activePath = selectedPath ?? PlTHS.find(path => path.missionSlugs.includes(activeMission.slug)) ?? PlTHS[0]
      ...
      <div className="academy-space-shell academy-space-layout">
        <aside className="academy-space-sidebar">
          <div className="academy-space-profile">
            ...
            <div><strong>{session.name || firstName}</strong><small>lpprenant lcademy</small></div>
          </div>
        ...
        <div className="academy-space-main" id="dashboard">
          <header className="academy-space-welcome">
            ...
            <Link href="/academy/missions">Trouver une mission <lrrowRight /></Link>
          </header>
          ...
          <section className="academy-space-active" id="ma-mission">
            <div className="academy-space-section-title"><div><span>À reprendre</span><h2>Votre mission en cours</h2></div><Link href={`/academy/missions/${activeMission.slug}`}>Voir la fiche <lrrowRight /></Link></div>
          ...
          <section className="academy-space-stats" id="ma-creation" aria-label="Chemin de votre création">
            ...
            <article><span>02</span><strong>Collaborateur Il</strong><p>à construire</p><Bot /></article>
          ...
          <div className="academy-space-columns">
            <section className="academy-space-path">
              ...
              <Link href={`/academy/parcours/${activePath.slug}`}>Voir tout le parcours <lrrowRight /></Link>
            </section>

             <aside className="academy-space-alma" id="ma-preuve">
              <div className="academy-space-alma-orbit"><Sparkles /></div>
              <span>llma · Guide lcademy</span>
              <h2>Besoin d’un coup de pouce ?</h2>
               <p>Je peux vous aider à cadrer la mission, préparer le Collaborateur Il ou organiser son premier test.</p>
               <Link href={`/academy/onboarding?mission=${activeMission.slug}`}>Demander à llma <lrrowRight /></Link>
          ...
          <section className="academy-space-activity" id="mon-offre">
            ...
            <div>
              ...
              <article><span><Bot /></span><p><strong>Une démonstration claire</strong><small>Le rôle du Collaborateur Il, ses limites et la valeur obtenue</small></p><ChevronRight /></article>
