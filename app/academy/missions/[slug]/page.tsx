...
export default async function Page({params}:{params:Promise<{slug:string}>}) {
  ...
  const nextHref=item.free?`/academy/onboarding?mission=${item.slug}`:'/academy/formations/co-createur-ia'
  return <main>
    <AcademyHero kicker={`Mission · ${sector?.name}`} title={item.title} body={item.result}/>
    <section className="px-5 pb-16 text-center"><Link href={nextHref} className="academy-button academy-button-primary gap-2">{item.free?'Préparer cette mission avec Alma':'Construire cette mission en formation'}<ArrowRight className="size-4"/></Link><p className="mx-auto mt-4 max-w-xl text-xs leading-5 text-[#857c6e]">{item.free?'Compte Unitalk requis · Aucun paiement · Votre progression reste dans votre espace':'Cette mission avancée est travaillée dans un parcours accompagné.'}</p></section>
