...
const TYPE_SLUG = 'competences'

export function generateStaticParams() {
  return STORE_ITEMS.filter((item) => item.type === 'competence').map((item) => ({ slug: item.slug }))
}

export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  const { slug } = await params
  const item = getStoreItem(TYPE_SLUG, slug)
  if (!item) return { title: 'Compétences · Unitalk' }
  return {
    title: `${item.name.fr} · ${TYPE_LABELS.competence.fr} · Unitalk`,
...
export default async function CompetenceDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params
  const item = getStoreItem(TYPE_SLUG, slug)
  if (!item) notFound()
  ...
  const breadcrumbJsonLd = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 2, name: 'Collaborateurs IA', item: `${SITE_URL}/collaborateurs-ia` },
      {
        '@type': 'ListItem',
        position: 3,
        name: TYPE_LABELS_PLURAL.competence.fr,
