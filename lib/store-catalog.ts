...
export const DOMAIN_LABELS: Record<string, Bilingual> = {
  ...
  donnees: { fr: 'Analyse de données', en: 'Data analysis' },
  'informatique-developpement': { fr: 'Informatique et développement', en: 'IT & development' },
  'cybersecurite-qualite-logicielle': { fr: 'Cybersécurité et DevOps', en: 'Cybersecurity & DevOps' },
...
const PROFILS: StoreItem[] = [
  {
    type: 'profil', slug: 'coordinatrice-missions', name: { fr: 'Coordinatrice de missions', en: 'Mission coordinator' }, description: { fr: 'Structure, affecte et suit les missions de l’entreprise.', en: 'Structures, assigns and tracks organization missions.' }, creator: 'unitalk', facet: 'transformation', roleInOrg: { fr: 'Socle opérationnel inclus avec la Licence Entreprise Unitalk.', en: 'Operational foundation included with the Unitalk Organization License.' }, knowHow: [{fr:'Clarifier le résultat attendu',en:'Clarify expected outcome'},{fr:'Sélectionner une mission',en:'Select a mission'},{fr:'Préparer les validations',en:'Prepare approvals'},{fr:'Organiser l’escalade',en:'Organize escalation'}], exampleMissions: [{fr:'Transformer un besoin en mission structurée',en:'Turn a need into a structured mission'}], relatedSkills: [], possibleApps: [], order: -1, dateAdded: '2026-08-13', keywords: ['alma','coordination','mission'], compatibleWith: ['alma'], includedWithLicense: 'organization',
  },
  {
    type: 'profil', slug: 'conseillere-adoption-ia', name: { fr: 'Conseillère en adoption IA', en: 'AI adoption advisor' }, description: { fr: 'Prépare les équipes, accompagne les premiers usages et traite les blocages qui freinent l’adoption des Collaborateurs IA.', en: 'Prepares teams, supports initial usage and addresses the blockers that slow down AI Collaborator adoption.' }, creator: 'unitalk', facet: 'transformation', roleInOrg: { fr: 'Aide les managers et les équipes métier à installer des usages utiles, compris et durables.', en: 'Helps managers and business teams establish useful, understood and lasting practices.' }, knowHow: [{fr:'Préparer les équipes au changement',en:'Prepare teams for change'},{fr:'Accompagner les premiers usages',en:'Support initial usage'},{fr:'Structurer les retours du terrain',en:'Structure field feedback'},{fr:'Identifier et traiter les blocages',en:'Identify and address blockers'}], exampleMissions: [{fr:'Accompagner le déploiement d’un Collaborateur IA dans une équipe',en:'Support the rollout of an AI Collaborator within a team'}], relatedSkills: [], possibleApps: [], order: 1, dateAdded: '2026-08-13', keywords: ['alma','adoption','formation','changement','déploiement'], compatibleWith: ['alma'],
  },
  {
    type: 'profil',
  ...
  },
  {
    ...
    creator: 'community',
    facet: 'informatique-developpement',
    roleInOrg: {
  ...
  ...[
    ...
    relatedSkills: MUSTAD_SKILLS.filter((skill) => skill.relatedProfiles?.includes(slug as string)).map((skill) => skill.slug),
    possibleApps: [],
    order: 300 + index,
...
const COMPETENCES: StoreItem[] = [
  ...
  ...[
    ...
    ['diagnostiquer-maturite-ia','Diagnostiquer la maturité IA','Évaluer usages, compétences, risques et opportunités de l’entreprise.'],
  ...
  ].map(([slug,name,description], index) => ({ type:'competence' as const, slug, name:{fr:name,en:name}, description:{fr:description,en:description}, creator:'unitalk' as const, facet:'operations', enables:[{fr:description,en:description}], produces:[{fr:'Un résultat documenté à valider',en:'A documented result to approve'}], contexts:[{fr:'Alma · Entreprise Unitalk',en:'Alma · Unitalk Organization'}], relatedProfiles:['coordinatrice-missions','conseiller-transformation-ia','conseillere-adoption-ia'], neededApps:[], order:50+index,dateAdded:'2026-08-13',keywords:[slug,'alma'],compatibleWith:['alma'] })),
  {
    type: 'competence',
  ...
  },
  {
    ...
    keywords: ['priorite', 'entreprise', 'tache', 'planning', 'todo'],
  },
  {
    type: 'competence',
...
const INTEGRATION_SOURCES: StoreItem[] = [
  ...
  },
  {
    ...
    description: {
      fr: 'Permet de consulter et organiser les documents des espaces autorisés de votre Entreprise.',
...
const SERVERS: StoreItem[] = [
  { type: 'server', slug: 'small', name: { fr: 'Small', en: 'Small' }, description: { fr: 'Serveur Unitalk AI Cloud pour démarrer avec un Collaborateur IA et des missions légères.', en: 'Unitalk AI Cloud server for getting started with one AI Collaborator and light workloads.' }, creator: 'unitalk', facet: 'unitalk-ai-cloud', enables: [{ fr: '1 CPU · 4 Go de RAM', en: '1 CPU · 4 GB RAM' }], contexts: [{ fr: 'Premiers usages et faible charge', en: 'First use cases and light workloads' }], permissions: [{ fr: 'Configuration gérée par Unitalk', en: 'Configuration managed by Unitalk' }], order: 0, dateAdded: '2026-08-16', keywords: ['small','1 cpu','4 go ram','unitalk ai cloud'], commercialStatus: 'included' },
  { type: 'server', slug: 'medium', name: { fr: 'Medium', en: 'Medium' }, description: { fr: 'Serveur Unitalk AI Cloud pour des missions régulières et plusieurs applications connectées.', en: 'Unitalk AI Cloud server for regular workloads and multiple connected applications.' }, creator: 'unitalk', facet: 'unitalk-ai-cloud', enables: [{ fr: '2 CPU · 8 Go de RAM', en: '2 CPUs · 8 GB RAM' }], contexts: [{ fr: 'Usage régulier et charge intermédiaire', en: 'Regular use cases and moderate workloads' }], permissions: [{ fr: 'Configuration gérée par Unitalk', en: 'Configuration managed by Unitalk' }], order: 1, dateAdded: '2026-08-16', keywords: ['medium','2 cpu','8 go ram','unitalk ai cloud'], commercialStatus: 'included' },
  { type: 'server', slug: 'large', name: { fr: 'Large', en: 'Large' }, description: { fr: 'Serveur accéléré pour les modèles exigeants, les automatisations continues et les équipes en production.', en: 'Accelerated server for demanding models, continuous automation and production teams.' }, creator: 'unitalk', facet: 'unitalk-ai-cloud', enables: [{ fr: '4 GPU · 16 Go de RAM', en: '4 GPUs · 16 GB RAM' }], contexts: [{ fr: 'Production et traitements accélérés', en: 'Production and accelerated workloads' }], permissions: [{ fr: 'Architecture et capacité validées avant activation', en: 'Architecture and capacity approved before activation' }], order: 2, dateAdded: '2026-08-16', keywords: ['large','4 gpu','16 go ram','unitalk ai cloud'], commercialStatus: 'included' },
  { type: 'server', slug: 'xl', name: { fr: 'XL', en: 'XL' }, description: { fr: 'Infrastructure GPU dédiée aux charges soutenues et à plusieurs Collaborateurs IA.', en: 'Dedicated GPU infrastructure for sustained workloads and multiple AI Collaborators.' }, creator: 'unitalk', facet: 'unitalk-ai-cloud', enables: [{ fr: '8 GPU · 32 Go de RAM', en: '8 GPUs · 32 GB RAM' }], contexts: [{ fr: 'Forte charge et exploitation multi-collaborateurs', en: 'Heavy workloads and multi-collaborator operations' }], permissions: [{ fr: 'Architecture et capacité validées avant activation', en: 'Architecture and capacity approved before activation' }], order: 3, dateAdded: '2026-08-16', keywords: ['xl','8 gpu','32 go ram','unitalk ai cloud'], commercialStatus: 'included' },
  { type: 'server', slug: 'xxl', name: { fr: 'XXL', en: 'XXL' }, description: { fr: 'Infrastructure GPU de très grande capacité pour les déploiements critiques et les volumes élevés.', en: 'Very high-capacity GPU infrastructure for critical deployments and high volumes.' }, creator: 'unitalk', facet: 'unitalk-ai-cloud', enables: [{ fr: '16 GPU · 64 Go de RAM', en: '16 GPUs · 64 GB RAM' }], contexts: [{ fr: 'Déploiement critique ou à grande échelle', en: 'Critical or large-scale deployment' }], permissions: [{ fr: 'Architecture, hébergement et capacité validés avant activation', en: 'Architecture, hosting and capacity approved before activation' }], order: 4, dateAdded: '2026-08-16', keywords: ['xxl','16 gpu','64 go ram','unitalk ai cloud'], commercialStatus: 'included' },
]
...
export function storeItemHref(item: StoreItem): string {
  ...
  if (item.type === 'server') return `/decouvrir?store=${item.slug}&source=marketplace-servers`
