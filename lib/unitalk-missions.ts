...
const executorLabel: Record<MissionExecutor, { fr: string; en: string }> = { alma: { fr: 'Alma', en: 'Alma' }, 'ai-cocreator': { fr: 'Co-créateur IA', en: 'AI Co-creator' }, 'ai-engineer': { fr: 'Ingénieur IA', en: 'AI Engineer' }, 'unitalk-team': { fr: 'Équipe Unitalk', en: 'Unitalk team' }, 'customer-ai-collaborator': { fr: 'Collaborateur IA de l’entreprise', en: 'Organization AI Collaborator' } }
...
export const UNITALK_MISSIONS: Mission[] = DEFINITIONS.map(([slug,title,description,subcategory,executor], index) => {
  ...
  return {
    slug:`unitalk-${slug}`, category:'unitalk', collections:['unitalk'], title:{fr:title,en:`Unitalk service: ${slug.replaceAll('-', ' ')}`}, result:{fr:description,en:'A scoped Unitalk service with prerequisites, deliverables and approval confirmed before execution.'}, description:{fr:description,en:'A scoped Unitalk service with prerequisites, deliverables and approval confirmed before execution.'}, objective:{fr:description,en:'Define the scope, prerequisites and expected outcome with Unitalk before execution.'}, steps:[{fr:'Alma ou l’intervenant autorisé précise le périmètre et les prérequis.',en:'The authorized advisor defines scope and prerequisites.'},{fr:'L’entreprise valide les crédits ou le devis avant toute exécution.',en:'The company approves credits or quote before execution.'}], deliverable:{fr:description,en:'A documented deliverable defined during scoping.'}, deliveryTime:{fr:'Confirmé après cadrage',en:'Confirmed after scoping'}, volume:{fr:'Défini lors du cadrage',en:'Defined during scoping'}, cadence:{fr:'Ponctuelle',en:'One-off'}, validation:{fr:'Aucun débit ni achat sans confirmation explicite.',en:'No debit or purchase without explicit confirmation.'}, produces:[{fr:description,en:'A documented outcome defined during scoping.'}], skills:[], tools:[], profile:executorLabel[executor], collaboratorSlug:'emma', sectors:['services'], languages:['fr'], zones:['france'], modalities:['configuration'], origin:'native', regulated:false, dateAdded:'2026-08-13', order:1000+index, keywords:[title.toLowerCase(),subcategory,'unitalk'], unitalk,
  }
})
