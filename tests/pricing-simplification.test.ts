...
describe('pricing simplification', () => {
  it('presents the organization license as a flat fee rather than per-seat pricing', () => {
    ...
    expect(sections).toContain('Modèles multimodaux, droits et validations')
  ...
  it('makes the AI Collaborator package explicit', () => {
    expect(sections).toContain('49 €/mois par Collaborateur')
  ...
  it('offers prepaid credits, BYOK and hybrid usage', () => {
    ...
    expect(sections).toContain('Des ressources incluses. Le reste à l’usage.')
  ...
  it('keeps the page focused and removes the configurator', () => {
    ...
    expect(sections).toContain("summaryTitle: '3. Votre total'")
  })
  ...
  it('makes the free first mission limits explicit', () => {
    ...
    expect(sections).toContain("kicker: 'Offre de lancement'")
    expect(sections).toContain('<Sparkles className="size-4')
