...
describe('Profils métier marketplace SEO', () => {
  ...
  it('uses a department-led catalog with decision-ready profile cards', () => {
    ...
    expect(hub).toContain('visibleProfiles = items.slice(0, visibleCount)')
    expect(hub).toContain("showMoreProfiles: 'Afficher 12 profils supplémentaires'")
    expect(hub).toContain('categoryTotal={profileCategoryTotal}')
    expect(hub).toContain('category.search[lang]')
