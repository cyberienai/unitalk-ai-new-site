...
describe('home commercial messaging', () => {
  it('uses a concrete organizational promise and an Alma personalization flow', () => {
    ...
    expect(hero).toContain('Mission après mission, il réutilise les méthodes validées et progresse avec votre entreprise.')
    expect(hero).toContain("voiceTitle: 'Quel travail voulez-vous confier à votre Collaborateur IA ?'")
  ...
  it('explains identity, organization and evolution without a separate comparison', () => {
    ...
    for (const copy of ["profile: 'Finance'", "'Relancer une facture'", "application: 'Outil de facturation'", "result: 'Suivi financier renforcé'"]) expect(sections).toContain(copy)
