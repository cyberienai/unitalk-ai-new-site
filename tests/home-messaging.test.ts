...
describe('home commercial messaging', () => {
  it('uses a concrete hero promise and recruitment reasons', () => {
    ...
    expect(hero).toContain('Décrivez votre besoin. Alma prépare votre Collaborateur.')
    expect(hero).toContain('Continuer avec cette mission')
    expect(hero).not.toContain('voiceSubmitEmpty')
    for (const proof of ['Première mission offerte', 'Première version en quelques minutes', 'Sans carte bancaire', 'Moteur Hermes open source', 'Infrastructure Unitalk hébergée en France']) expect(hero).toContain(proof)
  })
