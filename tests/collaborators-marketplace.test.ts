...
describe('Collaborateurs IA marketplace', () => {
  ...
  it('supports mission-led collaborator discovery while other catalogs retain search', () => {
    ...
    expect(hub).toContain('Une mission en tête ?')
    expect(hub).toContain('Alma prépare le bon Collaborateur.')
    ...
    expect(hub).not.toContain('Vous n’avez pas à choisir le modèle.')
