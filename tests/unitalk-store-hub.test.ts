...
describe('Marketplace IA hub', () => {
  ...
  it('shows canonical public AI Collaborators first', () => {
    ...
    expect(hub).toContain('collaboratorProfileHref(detail.slug, lang)')
  ...
  it('uses one crawlable horizontal category navigation', () => {
    ...
    expect(hub).toContain('<nav className="flex w-full overflow-x-auto pr-8 scrollbar-hide')
    expect(hub).toContain("href={localizedHref('missions', lang)}")
    expect(hub).toContain('overflow-x-auto pr-8 scrollbar-hide')
  ...
  it('frames models as access and servers as scalable execution infrastructure', () => {
    ...
    expect(hub).toContain('intention=hebergeur')
    expect(hub).toContain('function HostLogo')
    expect(hub).toContain("logoPath: '/logos/ovhcloud.svg'")
  ...
  it('shows popular proprietary and open-weight models with focused cards', () => {
    ...
    expect(hub).toContain('Demandez l’étude d’un modèle ou fournisseur grâce à un endpoint personnalisé.')
